...
get_script_dir <- function() {
  ...
  if (dir.exists(base_path)) {
    ...
    if (dir.exists(app_path) && file.exists(file.path(app_path, "deploy_auto.R"))) {
      return(app_path)
    }
  }
...
# 執行自動部署（非互動式）
deploy_with_config(config_file = "app_config.yaml", interactive = FALSE) 