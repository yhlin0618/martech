  ...
  if (apply_type_filter && length(type_values) > 0) {
    ...
    tryCatch({
      ...
      if (is.null(property_tbl)) {
        ...
        # Support both legacy "attributes" and new "property_name" column
        attr_col <- intersect(c("property_name", "attributes"), names(property_data))[1]
        if (is.na(attr_col)) {
          stop("Property table missing attribute column (property_name / attributes)")
        }
        attributes_to_keep <- property_data %>%
          dplyr::filter(type %in% !!type_values) %>%
          dplyr::pull(!!rlang::sym(attr_col)) %>%
          unique()
