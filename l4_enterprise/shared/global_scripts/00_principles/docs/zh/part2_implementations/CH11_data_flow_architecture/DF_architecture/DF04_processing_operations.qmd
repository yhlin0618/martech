...
# DF04: Processing Operations (3PR)

## Core Principle

Processing operations implement **business logic and analytical transformations**, converting transformed data into meaningful business metrics, segmentations, and derived insights. This is the **first phase of the DRV pipeline**, receiving data from ETL and applying business intelligence.
...
**Phase**: 3PR (Process)
**Position**: First phase of DRV pipeline, receives data from ETL (2TR)
...
graph LR
    ...
    D -.-> D1["3PR: Processing Layer<br/>• Business logic<br/>• KPI calculation<br/>• Customer segmentation<br/>• Aggregation"]

    style D fill:#fff8e1
    style D1 fill:#fff8e1
```

**Input**: `transformed_data.duckdb` (from ETL 2TR)
**Output**: `processed_data.duckdb` (to DRV 4CL)

### Why 3PR Comes First in DRV

Processing must happen **before** cleansing because:
1. **Business definitions first**: Need to calculate derived fields before deciding what's an "outlier"
2. **Segmentation before filtering**: Customer segments inform which cleaning rules apply
3. **Metrics before validation**: Business KPIs are calculated, then validated in 4CL
