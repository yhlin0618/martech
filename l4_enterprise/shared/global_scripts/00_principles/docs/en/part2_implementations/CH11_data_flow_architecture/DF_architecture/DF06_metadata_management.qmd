...
relates_to:
  ...
  - "ETL00": "ETL Framework"
  - "DM_R054": "Metadata Table Storage Pattern (v2)"
...
::: {.callout-important}
## Physical Storage Governed by DM_R054 v2 (2026-04-19)

Metadata tables (`df_platform`, `df_product_line`, `df_product_mapping`, and similar
reference / lookup tables) **MUST reside exclusively in `meta_data.duckdb`** — they
are **not** part of the 6-layer ETL flow, and must **not** be written to
`app_data.duckdb`.

- **Canonical producer**: `update_scripts/ETL/all/all_ETL_meta_init_0IM.R`
  (pre-autoinit bootstrap, non-rebuildable 7th layer parallel to `raw_data`).
- **Canonical reader**: `fn_load_product_lines()` (meta_data first, CSV seed only
  when bootstrapping from empty state).
- **Rebuild safety**: `rm data/app_data/app_data.duckdb && make run` rebuilds
  `app_data` without touching `meta_data`.

This DF06 document describes the *conceptual* metadata management layer (lineage,
schema documentation, quality metrics). For the *physical* storage contract and
migration history (v1 → v2), see
[`DM_R054`](../../../part1_principles/CH02_data_management/rules/DM_R054_metadata_table_storage_pattern.qmd).

Originating change: spectra change `metadata-to-meta-data-duckdb`
(GitHub issue [#422](https://github.com/kiki830621/ai_martech_global_scripts/issues/422)).
:::
