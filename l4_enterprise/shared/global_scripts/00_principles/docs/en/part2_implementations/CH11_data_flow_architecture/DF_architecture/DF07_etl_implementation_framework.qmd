...
### 6-Layer Symmetric Architecture (Current Standard)

Building on DF000 pipeline architecture, the ETL framework implements a comprehensive 6-layer symmetric data flow (ETL 3 + DRV 3):
...
graph TD
    ...
    B -.-> B1["Layer 0: Import (ETL)<br/>• Source-specific ingestion<br/>• Metadata preservation<br/>• Platform identification"]
    C -.-> C1["Layer 1: Staging (ETL)<br/>• File preprocessing<br/>• Type optimization<br/>• Encoding standardization"]
    D -.-> D1["Layer 2: Transform (ETL)<br/>• Structural JOINs<br/>• Schema mapping<br/>• No business logic"]
    E -.-> E1["Layer 3: Process (DRV)<br/>• Business logic<br/>• Derived calculations<br/>• Metric computation"]
    F -.-> F1["Layer 4: Cleanse (DRV)<br/>• Data quality<br/>• Outlier handling<br/>• Missing values"]
    G -.-> G1["Layer 5: Normalize (DRV)<br/>• Statistical normalization<br/>• z-score, min-max<br/>• App-ready delivery"]
```

### Layer Definitions and Responsibilities (6-Layer Symmetric)

| Layer | Phase | Code | Pipeline | Purpose | Database |
|-------|-------|------|----------|---------|----------|
| **0** | Import | 0IM | ETL | External data ingestion | raw_data.duckdb |
| **1** | Staging | 1ST | ETL | Format standardization | staged_data.duckdb |
| **2** | Transform | 2TR | ETL | Structural JOINs | transformed_data.duckdb |
| **3** | Process | 3PR | DRV | Business logic | processed_data.duckdb |
| **4** | Cleanse | 4CL | DRV | Data quality | cleansed_data.duckdb |
| **5** | Normalize | 5NM | DRV | App-ready delivery | app_data.duckdb |
...
execute_etl_phase <- function(etl_series, phase_code, phase_name, config) {
  ...
  tryCatch({
    ...
    # Phase-specific logic (6-Layer Symmetric Pipeline)
    result <- switch(phase_code,
      ...
      "5NM" = execute_normalize_phase(config)
    )
...
etl_config_template <- list(
  ...
  # Normalization configuration (Phase 5NM)
  # Note: 5DN and 6NM were merged into 5NM in the 6-layer revision (2025-12-24)
  normalization = list(
...
- **DF000 (Pipeline Architecture)**: Implements the theoretical pipeline as 6-layer symmetric ETL+DRV
