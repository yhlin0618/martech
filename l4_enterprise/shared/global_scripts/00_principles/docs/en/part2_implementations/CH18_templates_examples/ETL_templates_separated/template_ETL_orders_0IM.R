...
main_execution <- function() {
  tryCatch({
    ...
    orders_processed <- process_orders_import(orders_raw_data, platform_id = "{platform}")
    ...
    validation_result <- validate_orders_import(orders_standardized, platform_id = "{platform}")
...
test_execution <- function() {
  ...
  required_fields <- c("order_id", "customer_id", "order_status", "total_amount", "platform_id", "import_timestamp")
