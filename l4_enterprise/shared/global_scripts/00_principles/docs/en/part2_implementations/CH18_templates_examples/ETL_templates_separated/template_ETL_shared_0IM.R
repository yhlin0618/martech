...
main_execution <- function() {
  tryCatch({
    ...
    sales_raw <- extract_sales_data(complete_api_response, platform_id = "{platform}")
    customers_raw <- extract_customers_data(complete_api_response, platform_id = "{platform}")
    orders_raw <- extract_orders_data(complete_api_response, platform_id = "{platform}")
    products_raw <- extract_products_data(complete_api_response, platform_id = "{platform}")
      ...
      if (DBI::dbExistsTable(raw_data, table_name)) {
        ...
        if (length(platform_ids) != 1 || platform_ids[1] != "{platform}") {
          stop(sprintf("Platform ID inconsistent in %s. Expected: {platform}, Found: %s", 
                       table_name, paste(platform_ids, collapse = ", ")))
        }
      }
    }
