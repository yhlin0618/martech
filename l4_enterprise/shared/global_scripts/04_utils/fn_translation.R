...
initialize_translation_system <- function(language = "en", translation_dict = NULL) {
  ...
  # Vectorized: works with scalar OR character vector input (e.g. dplyr::rename_with)
  translate <- function(text, lang = NULL) {
    # Use specified language or fall back to current language
    use_lang <- if (is.null(lang)) current_language else lang

    # If no dictionary available or language not found, return original text
    if (is.null(dict) || !is.list(dict) || is.null(dict[[use_lang]])) {
      return(text)
    }

    lang_dict <- dict[[use_lang]]

    # Vectorized lookup for named character vector dictionary
    if (is.character(lang_dict)) {
      hits <- match(text, names(lang_dict))
      out <- ifelse(is.na(hits), text, unname(lang_dict[hits]))
      return(out)
    }

    # Vectorized lookup for list-type dictionary
    if (is.list(lang_dict)) {
      out <- vapply(text, function(t) {
        val <- lang_dict[[t]]
        if (is.null(val)) t else as.character(val)
      }, character(1), USE.NAMES = FALSE)
      return(out)
    }

    # Unknown dict type → passthrough
    text
  }
