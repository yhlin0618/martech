  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$dna_plot <- renderPlotly({
      ...
      for (i in seq_along(product_id_groups)) {
        ...
        # Determine visibility based on settings (fallback TRUE if input missing)
        show_all_val <- isTRUE(input$show_all_initially)
        visibility <- if (show_all_val) TRUE else "legendonly"
