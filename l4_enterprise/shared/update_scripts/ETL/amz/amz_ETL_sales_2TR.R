...
tryCatch({
  ...
  # Step 2.5: Derive customer_id (#359)
  # Amazon Seller Central does not expose buyer_email in newer exports, so we
  # use a fallback strategy:
  #   - Primary: LOWER(local part of buyer_email) if buyer_email column exists
  #   - Fallback: ship_country + "_" + ship_postal_code (stable proxy per household)
  # Rows with neither identifier are dropped at the filter step below.
  message("MAIN: Step 2.5/4 - Deriving customer_id...")
  id_start <- Sys.time()

  has_email <- "buyer_email" %in% names(dt) &&
    any(!is.na(dt$buyer_email) & nzchar(dt$buyer_email))

  if (has_email) {
    dt[, customer_id := {
      email <- tolower(trimws(as.character(buyer_email)))
      local <- sub("@.*$", "", email)
      ifelse(!is.na(local) & nzchar(local), local, NA_character_)
    }]
    n_from_email <- sum(!is.na(dt$customer_id))
    message(sprintf("    Derived %d customer_ids from buyer_email", n_from_email))
  } else {
    dt[, customer_id := NA_character_]
    message("    buyer_email not available; using ship_postal_code fallback")
  }

  # Fallback: ship_country + ship_postal_code for rows still missing customer_id
  if ("ship_postal_code" %in% names(dt)) {
    country_col <- if ("ship_country" %in% names(dt)) "ship_country" else NA_character_
    need_fallback <- is.na(dt$customer_id) & !is.na(dt$ship_postal_code) &
      nzchar(as.character(dt$ship_postal_code))
    if (any(need_fallback)) {
      zip_part <- trimws(as.character(dt$ship_postal_code[need_fallback]))
      country_part <- if (!is.na(country_col)) {
        trimws(as.character(dt[[country_col]][need_fallback]))
      } else {
        rep("XX", sum(need_fallback))
      }
      country_part[is.na(country_part) | !nzchar(country_part)] <- "XX"
      dt$customer_id[need_fallback] <- paste0(country_part, "_", zip_part)
      message(sprintf("    Derived %d customer_ids from ship_postal_code fallback",
                      sum(need_fallback)))
    }
  }

  n_have_id <- sum(!is.na(dt$customer_id))
  message(sprintf("MAIN: customer_id coverage: %d/%d (%.1f%%) (%.2fs)",
                  n_have_id, nrow(dt), 100 * n_have_id / nrow(dt),
                  as.numeric(Sys.time() - id_start, units = "secs")))
  ...
  # Filter: remove records with NA customer_id (#359)
  n_before <- nrow(dt)
  dt <- dt[!is.na(customer_id) & nzchar(customer_id)]
  n_removed <- n_before - nrow(dt)
  if (n_removed > 0) {
    message(sprintf("    Removed %d records with NA customer_id", n_removed))
  }
