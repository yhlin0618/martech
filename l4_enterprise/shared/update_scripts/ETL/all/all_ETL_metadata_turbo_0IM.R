...
# metadata_sources.turbo is an optional company-specific feature (MAMBA only).
# When not configured, skip gracefully so {targets} treats this as success.
turbo_enabled <- !is.null(turbo_company) && isTRUE(turbo_company$enabled)
if (!turbo_enabled) {
  message("[all_ETL_metadata_turbo_0IM] metadata_sources.turbo not enabled — skipping (not applicable for this company)")
  final_status <- TRUE
  autodeinit()
} else {
