...
# Determine which product lines have upstream data (df_comment_property_ratingonly_*)
# Product lines without reviews won't have this source table — skip them gracefully.
eligible_lines <- character()
skipped_no_source <- character()
for (product_line_id_i in vec_product_line_id_noall) {
  src_table <- paste0("df_comment_property_ratingonly_", product_line_id_i)
  if (DBI::dbExistsTable(processed_data, src_table)) {
    eligible_lines <- c(eligible_lines, product_line_id_i)
  } else {
    skipped_no_source <- c(skipped_no_source, product_line_id_i)
  }
}

if (length(skipped_no_source) > 0) {
  message("\nD03_07 skip (no upstream ratings data): ",
          paste(skipped_no_source, collapse = ", "))
}

# Process each eligible product line
success_count <- 0
failed_lines <- character()
for (product_line_id_i in eligible_lines) {
  message("\n==============================")
...
message("- Total eligible product lines: ", length(eligible_lines))
message("- Successfully processed: ", success_count)
message("- Failed: ", length(eligible_lines) - success_count)
message("- Skipped (no upstream data): ", length(skipped_no_source))

# Output verification: Check created tables for eligible lines only
