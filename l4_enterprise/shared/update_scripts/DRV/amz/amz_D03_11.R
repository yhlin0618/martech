...
# Filter to product lines that have upstream ratings data
eligible_lines <- character()
skipped_no_source <- character()
for (product_line_id_i in vec_product_line_id_noall) {
  src_table <- paste0("df_comment_property_ratingonly_by_asin_", product_line_id_i)
...
if (length(skipped_no_source) > 0) {
  message("D03_11 skip (no upstream data): ", paste(skipped_no_source, collapse = ", "))
}
...
# Phase 2: Merge all eligible product line data
merge_success <- merge_position_tables(
  product_line_ids = eligible_lines,
  app_data = app_data,
  paste_ = paste_
)
