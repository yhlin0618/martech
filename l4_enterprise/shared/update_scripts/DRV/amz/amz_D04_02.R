...
#####
#P07_D04_02
# DERIVATION: AMZ Poisson Analysis
# VERSION: 5.0
# PLATFORM: amz
# GROUP: D04
# SEQUENCE: 02
# PURPOSE: Pre-compute Poisson regression results using ALL historical data
# CONSUMES: df_amz_sales_complete_time_series_{product_line}
# PRODUCES: df_amz_poisson_analysis_{product_line}, df_amz_poisson_analysis_all
# PRINCIPLE: DM_R044, MP064, MP135, DM_R046, R118, R119, R120, DM_R043
#####

#amz_D04_02

#' @title AMZ Poisson Analysis - Type B Steady-State Analytics
...
#' @output_tables df_amz_poisson_analysis_{product_line}, df_amz_poisson_analysis_all
...
cat("\n")
cat("════════════════════════════════════════════════════════════════════\n")
cat("AMZ Product-Line Poisson DRV - Type B Steady-State (MP135 v2.0)\n")
cat("════════════════════════════════════════════════════════════════════\n")
...
# Product lines to process — config-driven (#363)
...
for (pl in PRODUCT_LINES) {
  ...
  table_name <- sprintf("df_amz_sales_complete_time_series_%s", pl)
  output_table_name <- sprintf("df_amz_poisson_analysis_%s", pl)
  skip_reason <- NULL
  ...
  exclude_cols <- c(
    "amz_item_id", "amz_asin", "time", "sales", "sales_platform",
...
if (length(all_results) > 0) {
  ...
  dbWriteTable(con_app, "df_amz_poisson_analysis_all", merged_table, overwrite = TRUE)
  cat("\n  ✅ Wrote to: df_amz_poisson_analysis_all\n")

} else {
  cat("  ⚠️  No results to merge\n")
  dbWriteTable(con_app, "df_amz_poisson_analysis_all", empty_output_table, overwrite = TRUE)
  cat("  → Wrote empty schema to: df_amz_poisson_analysis_all\n")
}
...
for (pl in PRODUCT_LINES) {
  table_name <- sprintf("df_amz_poisson_analysis_%s", pl)
...
if (dbExistsTable(con_app, "df_amz_poisson_analysis_all")) {
  row_count <- tbl2(con_app, "df_amz_poisson_analysis_all") %>%
    summarise(n = dplyr::n()) %>%
    collect() %>%
    dplyr::pull(n)

  # Verify Type B metadata columns
  cols <- dbListFields(con_app, "df_amz_poisson_analysis_all")
  has_computed_at <- "computed_at" %in% cols
  has_data_version <- "data_version" %in% cols

  cat(sprintf("  ✓ df_amz_poisson_analysis_all: %d predictors", row_count))
  if (has_computed_at && has_data_version) {
    cat(" (Type B metadata ✓)\n")
  } else {
    cat(" (⚠️ Missing Type B metadata)\n")
  ...
  cat("  ❌ df_amz_poisson_analysis_all: NOT FOUND\n")
  validation_pass <- FALSE
}
...
for (pl in PRODUCT_LINES) {
  cat(sprintf("    - df_amz_poisson_analysis_%s\n", pl))
}
cat("  app_data.duckdb (for UI):\n")
cat("    - df_amz_poisson_analysis_all (merged)\n")
cat("\n")
