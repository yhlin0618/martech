...
if (!exists("df_product_line", envir = .GlobalEnv, inherits = TRUE) &&
    exists("GLOBAL_PARAMETER_DIR", envir = .GlobalEnv, inherits = TRUE) &&
    exists("APP_PARAMETER_DIR", envir = .GlobalEnv, inherits = TRUE)) {
  csv_candidates <- c(
    file.path(APP_PARAMETER_DIR, "scd_type1", "df_product_line.csv"),
    file.path(GLOBAL_PARAMETER_DIR, "scd_type1", "df_product_line.csv")
  )
  csv_candidates <- csv_candidates[file.exists(csv_candidates)]
  if (length(csv_candidates) == 0) {
    stop("UPDATE_MODE: df_product_line.csv not found in APP_PARAMETER_DIR or GLOBAL_PARAMETER_DIR")
  }

  df_product_line <- read.csv(csv_candidates[[1]], stringsAsFactors = FALSE)
