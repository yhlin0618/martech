      ...
      for (f in files) {
        ...
        if (length(matches) >= 3) {
          ...
          if (length(prev_seqs) > 0) {
            ...
            if (group_name == "D01") {
              sales_files <- list.files(platform_etl_dir, pattern = "_ETL_sales_")
              sales_files <- sort(sales_files)

              sales_2ts <- sales_files[grepl("_ETL_sales_2TS", sales_files)]
              sales_2tr <- sales_files[grepl("_ETL_sales_2TR", sales_files)]

              preferred_sales_dep <- c(sales_2ts, sales_2tr)
              if (length(preferred_sales_dep) > 0) {
                script_entry$depends_etl <- script_to_target_name(preferred_sales_dep[[1]])
              } else {
