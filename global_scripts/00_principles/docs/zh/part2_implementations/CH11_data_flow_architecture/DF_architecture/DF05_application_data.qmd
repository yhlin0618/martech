---
id: "DF005"
title: "Application Data (5NM + APP)"
type: "data-flow"
...
# DF05: Application Data (5NM + APP)

## Core Principle

This layer combines **statistical normalization (5NM)** and **application data preparation (APP)**, transforming cleansed data into UI-optimized formats. This is the **final phase of the DRV pipeline**, delivering data ready for application consumption.

## Purpose and Scope

### What This Layer Does

**5NM (Normalization) Functions:**
- **Statistical normalization**: Z-score, Min-Max scaling for ML/visualization
- **Feature scaling**: Prepare numeric features for consistent display
- **Categorical encoding**: Convert categories to app-friendly formats
...
- **Caching strategy implementation**: Create cacheable data structures
- **API endpoint preparation**: Format data for microservice consumption

### What This Layer Does NOT Do
- **Business logic calculations**: No KPIs or metrics (handled in DF04/3PR)
- **Data quality fixes**: No validation or cleansing (handled in DF03/4CL)
- **Real-time updates**: No live data streaming (handled at application runtime)

## Layer Position in Pipeline

**Pipeline**: DRV (Data Intelligence)
**Phase**: 5NM (Normalize) → APP (Deliver)
**Position**: Final phase of DRV pipeline, receives data from 4CL (Cleanse)
...
graph LR
    ...
    subgraph DRV["DRV Pipeline"]
        C --> D[processed_data]
        D --> E[cleansed_data]
        E --> F[app_data]
    end

    F -.-> F1["5NM + APP Layer<br/>• Statistical normalization<br/>• UI optimization<br/>• Performance caching<br/>• API preparation"]

    style F fill:#f3e5f5
    style F1 fill:#f3e5f5
```

**Input**: `cleansed_data.duckdb` (from DRV 4CL)
**Output**: `app_data.duckdb` (final delivery)

### Why 5NM Comes Last

Statistical normalization must happen **after** cleansing because:
1. **Outlier sensitivity**: z-score/min-max are distorted by outliers (removed in 4CL)
2. **Clean baseline**: Normalization should use clean data distributions
3. **Final step**: Normalized values are app-specific and shouldn't be further processed
...
## Metadata Tables in app_data (DM_R054)

In addition to data flowing through the full 6-layer pipeline, **metadata tables** write directly to `app_data.duckdb`, bypassing the ETL layers.

### Two Data Paths to app_data

```
Path 1: Transaction Data (full pipeline)
  0IM → 1ST → 2TR → 3PR → 4CL → 5NM → app_data

Path 2: Metadata Tables (direct)
  source → app_data (single-phase ETL)
```

### Metadata Tables in app_data

| Table | Purpose | Source |
|-------|---------|--------|
| `df_product_mapping` | Cross-platform product ID mapping | Google Sheets, Excel |
| `df_product_line` | Product line definitions | Google Sheets |
| `df_platform` | Platform IDs and names | Config |
| `df_variable_metadata` | UI display names | Config |
| `df_customer_labels` | Customer segment labels | Config |
| `df_config_*` | Application configurations | YAML |

### Why Metadata Skips the Pipeline
...
# Metadata writes directly to app_data, not through 6-layer ETL
...
See **DM_R054: Metadata Table Storage Pattern** for the complete specification.
...
The app_data layer serves as the final consumption point that bridges processed business intelligence with real-time user interactions in modern web applications. Additionally, app_data stores **metadata tables** directly (per DM_R054), providing reference data that applications need at startup.