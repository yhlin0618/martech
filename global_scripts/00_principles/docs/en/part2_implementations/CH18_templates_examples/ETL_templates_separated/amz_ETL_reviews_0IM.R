...
main_execution <- function() {
  tryCatch({
    ...
    reviews_processed <- process_reviews_import(reviews_raw_data, platform_id = "amz")
    ...
    validation_result <- validate_reviews_import(reviews_standardized, platform_id = "amz")
...
test_execution <- function() {
  tryCatch({
    ...
    required_fields <- c("review_id", "product_id", "customer_id", "rating", 
                         "review_date", "review_text", "platform_id", 
    ...
    if (length(platform_ids) != 1 || platform_ids[1] != "amz") {
