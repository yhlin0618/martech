...
main_execution <- function() {
  tryCatch({
    ...
    products_processed <- process_products_import(products_raw_data, platform_id = "{platform}")
    ...
    validation_result <- validate_products_import(products_standardized, platform_id = "{platform}")
...
test_execution <- function() {
  ...
  required_fields <- c("product_id", "product_name", "category", "sku", "platform_id", "import_timestamp")
  ...
  platform_check <- dbGetQuery(raw_data, sprintf("SELECT DISTINCT platform_id FROM %s", table_name))
