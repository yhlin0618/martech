...
**Phase**: 4CL (Cleanse)
**Position**: Second phase of DRV pipeline, after 3PR (Process)
...
graph LR
    ...
    E -.-> E1["4CL: Cleansing Layer<br/>• Data quality assurance<br/>• Deduplication<br/>• Missing value handling<br/>• Validation"]

    style E fill:#e8f5e8
    style E1 fill:#e8f5e8
```

**Input**: `processed_data.duckdb` (from 3PR)
**Output**: `cleansed_data.duckdb` (to 5NM)

### Why 4CL Comes After 3PR

Cleansing must happen **after** business processing because:
1. **Business context needed**: Cleaning rules often depend on business definitions (e.g., "test orders" must be identified by business logic first)
2. **Derived fields**: Outlier detection may apply to calculated metrics from 3PR
3. **Business validity**: Some records are valid raw data but invalid business entities
