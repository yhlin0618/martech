...
## Spec-First Governance for DRV Changes {#spec-first-governance}

This standard governs both behavior and implementation in DRV. DRV scripts are execution artifacts, not policy source.

Before making behavioral changes in a DRV script, follow this sequence:

1. **Stop direct script edits** immediately when a behavior gap, conflict, or exception request is found.
2. **Identify all governing documents**:
   - The group task's `_implementation_rules.yaml`
   - The task documentation (`Dxx_*_*.qmd`) that describes required behavior
   - Relevant MP/P/R references (e.g., `MP028`, `MP037`, `DM_R044`, `DEV_R032`)
3. **Update governance documentation first**:
   - align scope, edge cases, definitions, and precedence
   - record rationale and migration notes
   - define acceptance criteria and rollback conditions
4. **Obtain governance approval** when process requires review.
5. **Modify the DRV script only after** documentation alignment is complete.

### DRV Compliance Rule

- No new script behavior may be introduced before task spec and `_implementation_rules.yaml` are updated.
- If an exception is required, it must first be documented as a rule/principle amendment.
- Every behavioral change in DRV code must be traceable to updated governing documentation.

### DRV Governance Checklist

- [ ] Is the applicable `_implementation_rules.yaml` updated first?
- [ ] Are affected task docs updated with scope and precedence?
- [ ] Has the governing MP/P/R mapping been declared?
- [ ] Is the code change only implementing updated spec?
...
# CONSUMES: df_{platform}_sales___standardized, df_customer_standardized
# PRODUCES: df_dna_by_customer, df_segments_by_customer
...
| **CONSUMES** | Input tables from ETL | "df_{platform}_sales___standardized" |
| **PRODUCES** | Output tables created | "df_dna_by_customer" |
...
#' @input_tables
#'   - processed_data.df_{platform}_sales___standardized (from ETL01_2TR)
#'   - processed_data.df_customer_standardized (from ETL02_2TR)
#' @output_tables
#'   - app_data.df_dna_by_customer (customer profiles)
#'   - app_data.df_segments_by_customer (segmentation results)
#' @business_rules
...
tryCatch({
  ...
  df_sales <- tbl(processed_data, "df_{platform}_sales___standardized") %>%
  ...
  rows_processed <- nrow(df_dna_by_customer)
  ...
  message(sprintf("Wrote %d customer profiles to df_dna_by_customer",
                 rows_processed))

  # 2.6: Create Summary Statistics
  segment_summary <- df_dna_by_customer %>%
...
if (!error_occurred) {
  tryCatch({
    ...
    if (!dbExistsTable(app_data, "df_dna_by_customer")) {
      stop("Primary output table df_dna_by_customer was not created")
    }

    if (!dbExistsTable(app_data, "df_segments_by_customer")) {
      stop("Summary table df_segments_by_customer was not created")
    }

    # 3.2: Validate Output Structure
    dna_sample <- tbl(app_data, "df_dna_by_customer") %>%
      head(5) %>%
      collect()
    ...
    invalid_rfm <- tbl(app_data, "df_dna_by_customer") %>%
    ...
    negative_revenue <- tbl(app_data, "df_dna_by_customer") %>%
    ...
    output_rows <- tbl(app_data, "df_dna_by_customer") %>%
      summarize(n = n()) %>%
      pull(n)
    ...
    segment_distribution <- tbl(app_data, "df_segments_by_customer") %>%
      collect()
...
if (test_passed && !error_occurred) {
  ...
  segments <- tbl(app_data, "df_segments_by_customer") %>% collect()
...
get_platform_info <- function(script_name = NULL) {
  ...
  platform_id <- substr(script_name, 1, 3)

  if (!platform_id %in% names(VALID_PLATFORMS)) {
    stop(sprintf("Invalid platform_id: %s", platform_id))
  }

  return(VALID_PLATFORMS[[platform_id]])
}
```
...
process_all_platforms <- function() {
  ...
  for (platform in platforms) {
    tryCatch({
      ...
      platform_data <- tbl(processed_data, "df_{platform}_sales___standardized") %>%
        filter(platform_id == !!platform) %>%
        collect()
...
# Error indicators
message("❌ DM_R044 ERROR: Required table 'df_{platform}_sales___standardized' not found")
...
# PRODUCES: df_profile_by_customer
# PRINCIPLE: MP064, DM_R042, DM_R044
#####
#P07_D01_03
```
...
#' @output_tables app_data.df_profile_by_customer
#' @platform cbz
#' @date 2025-09-29
```
...
9. **Apply spec-first governance** before DRV behavior changes
...
9. **Don't modify DRV behavior** before governing docs and rules are updated
...
```
00_principles/docs/en/part2_implementations/CH13_derivations/TEMPLATE_derivation.R
```
...
```
00_principles/docs/en/part2_implementations/CH13_derivations/validate_derivation_format.R
```
...
|------|---------|---------|
| 2026-02-13 | 1.2 | Added Spec-First governance workflow for DRV behavior changes before code modifications |
