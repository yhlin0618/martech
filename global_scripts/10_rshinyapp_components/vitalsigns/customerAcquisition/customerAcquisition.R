...
customerAcquisitionComponent <- function(id, app_connection, comp_config, translate) {
  ...
  ui_display <- tagList(
    ...
    fluidRow(
      ...
      column(3, uiOutput(ns("kpi_conversion")))
    ),
    # Charts Row
    fluidRow(
  ...
  server_fn <- function(input, output, session) {
    moduleServer(id, function(input, output, session) {
      ...
      dna_data <- reactive({
        ...
        tryCatch({
          ...
          df <- DBI::dbGetQuery(app_connection, paste0(
            "SELECT d.customer_id, d.nes_status, d.f_value, d.ni AS ni_count, d.m_value, d.total_spent AS spent_total,",
            " d.r_value, d.clv AS clv_value, d.nrec_prob",
          ...
          message("[customerAcquisition] UI targets: kpi_active, kpi_total, kpi_new_rate, kpi_conversion, structure_pie, funnel_chart, detail_table")
          df
        }, error = function(e) {
      ...
      output$kpi_conversion <- renderUI({
        df <- dna_data()
        if (is.null(df)) return(bs4ValueBox(value = "-", subtitle = translate("Customer Change Rate"),
                                            icon = icon("redo"), color = "warning", width = 12))
        n_repeat <- sum(df$ni_count >= 2, na.rm = TRUE)
        pct <- round(n_repeat / nrow(df) * 100, 1)
        bs4ValueBox(value = paste0(pct, "%"), subtitle = translate("Customer Change Rate"),
                    icon = icon("redo"), color = "warning", width = 12)
      })
