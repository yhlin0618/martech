# =============================================================
# positionTable_production_test.R
...
# ---- 1. 套件 --------------------------------------------------------------
library(shiny); library(bs4Dash); library(plotly); library(dplyr); library(shinyjs); library(DBI); library(DT)
...
server <- function(input, output, session){
  ...
  comp_config <- reactive({
    list(
      filters    = list(platform_id = input$platform,
                        product_line_id_chosen = input$product_line),
  ...
  observeEvent(input$sidebar_menu, {
    ...
    if (input$sidebar_menu %in% c("dna", "position")) {
