...
customerActivityComponent <- function(id, app_connection, comp_config, translate) {
  ...
  server_fn <- function(input, output, session) {
    moduleServer(id, function(input, output, session) {
      ...
      # Histogram: CAI distribution
      output$hist_cai <- renderPlotly({
        df <- dna_data()
        if (is.null(df)) return(plotly::plot_ly() %>% plotly::layout(title = translate("No Data")))
        vals <- df$cai[!is.na(df$cai)]
        ...
        plotly::plot_ly(x = vals, type = "histogram",
          ...
          plotly::layout(xaxis = list(title = translate("CAI")), yaxis = list(title = translate("Customer Count")),
