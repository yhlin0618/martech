...
customerValueComponent <- function(id, app_connection, comp_config, translate) {
  ...
  server_fn <- function(input, output, session) {
    moduleServer(id, function(input, output, session) {
      ...
      # Histogram helper — DP_R001: cap extreme values for readability
      # cap_discrete: fixed upper bound for count data (e.g. F capped at 50)
      # cap_pctl: percentile cap for continuous data (e.g. M capped at 99th pctl)
      render_hist <- function(col, xlab, color, cap_discrete = NULL, cap_pctl = NULL) {
        renderPlotly({
          df <- dna_data()
          ...
          if (!is.null(cap_discrete)) {
            ...
            if (n_over > 0) {
              annotation <- list(
                text = paste0(cap_discrete, "+ (", format(n_over, big.mark = ","), " ", translate("customers"), ")"),
          ...
          p <- plotly::plot_ly(x = vals, type = "histogram",
            ...
            plotly::layout(xaxis = list(title = xlab), yaxis = list(title = translate("Customer Count")),
      ...
      output$hist_r <- render_hist("r_value", translate("Recency (days)"), "#dc3545", cap_pctl = 0.99)
      output$hist_f <- render_hist("f_value", translate("Purchase Frequency"), "#28a745", cap_discrete = 50)
      output$hist_m <- render_hist("m_value", translate("Purchase Amount"), "#007bff", cap_pctl = 0.99)
