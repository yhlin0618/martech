      ...
      output$hist_ipt <- renderPlotly({
        ...
        plotly::plot_ly(x = vals, type = "histogram",
          ...
          plotly::layout(
            ...
            annotations = list(
              list(x = p20, y = 1, yref = "paper", text = paste0("P20=", round(p20, 0)), showarrow = FALSE, yanchor = "bottom"),
              list(x = p50, y = 0.9, yref = "paper", text = paste0("P50=", round(p50, 0)), showarrow = FALSE, yanchor = "bottom"),
              list(x = p80, y = 0.8, yref = "paper", text = paste0("P80=", round(p80, 0)), showarrow = FALSE, yanchor = "bottom")
            ),
            margin = list(t = 10, b = 50)
          )
      })

      # Histogram: M distribution (cap at P99 to handle extreme outliers — Issue #245)
      output$hist_m <- renderPlotly({
        df <- dna_data()
        if (is.null(df)) return(plotly::plot_ly() %>% plotly::layout(title = translate("No Data")))
        vals <- df$m_value[!is.na(df$m_value)]
        if (length(vals) == 0) return(plotly::plot_ly() %>% plotly::layout(title = translate("No Data")))
        cap <- stats::quantile(vals, 0.99)
        vals_display <- vals[vals <= cap]
        n_excluded <- length(vals) - length(vals_display)
        subtitle <- if (n_excluded > 0) paste0(" (", n_excluded, " outliers > $", round(cap, 0), " excluded)") else ""
        plotly::plot_ly(x = vals_display, type = "histogram",
                        marker = list(color = "#17a2b8", line = list(color = "white", width = 1))) %>%
          plotly::layout(xaxis = list(title = paste0(translate("Purchase Amount"), subtitle)),
                         yaxis = list(title = translate("Customer Count")),
                         margin = list(t = 10, b = 50))
      })
      ...
      output$detail_table <- DT::renderDataTable({
        ...
        DT::datatable(show_df,
          colnames = c(translate("Customer ID"), translate("Avg Purchase Interval"),
                       translate("M"), translate("F"), translate("PCV"), translate("CRI"), translate("Total Spent")),
