...
if (!exists("df_platform", inherits = TRUE)) {
  df_platform_candidates <- c(
    file.path("data", "app_data", "parameters", "scd_type1", "df_platform.csv"),
    file.path("scripts", "global_scripts", "30_global_data", "parameters", "scd_type1", "df_platform.csv")
  )
  df_platform_path <- df_platform_candidates[file.exists(df_platform_candidates)][1]
  if (is.na(df_platform_path)) {
    stop("df_platform.csv not found in expected paths")
  }
  df_platform <- read.csv(df_platform_path, stringsAsFactors = FALSE)
}

if (!exists("df_product_line", inherits = TRUE)) {
  df_product_line_candidates <- c(
    file.path("data", "app_data", "parameters", "scd_type1", "df_product_line.csv"),
    file.path("scripts", "global_scripts", "30_global_data", "parameters", "scd_type1", "df_product_line.csv")
  )
  df_product_line_path <- df_product_line_candidates[file.exists(df_product_line_candidates)][1]
  if (is.na(df_product_line_path)) {
    stop("df_product_line.csv not found in expected paths")
  }
  df_product_line <- read.csv(df_product_line_path, stringsAsFactors = FALSE)
}
...
product_line_choices_df <- tryCatch({
  df_product_line[which(as.logical(df_product_line$included)), , drop = FALSE]
}, error = function(e) {
  df_product_line
})
...
main_app_ui <- bs4DashPage(
  ...
  sidebar = bs4DashSidebar(status="primary", width="300px", elevation = 3,minified  = FALSE,
                           ...
                           # Local filter: 只在特定 tab 出現（如 VitalSigns 的國家篩選）
                           uiOutput("country_filter_sidebar"),
                           # 元件專屬 filter 容器（AI 按鈕在此區最底部）
...
server <- function(input, output, session){
  ...
  comp_config <- reactive({
    ...
    vs_country <- input$vs_country
    if (is.null(vs_country) || !nzchar(as.character(vs_country))) {
      vs_country <- "all"
    }
    ...
    list(
      filters    = list(platform_id = input$platform,
                        ...
                        # UI selection vs internal filtered state are intentionally separated.
  ...
  # ---- 6.2b Country filter (Issue #237) -----------------------------------
  # Local filter: 只在 VitalSigns tab 顯示（UI_R026 Sidebar Layout Hierarchy）
  vs_tabs <- c("revenuePulse", "customerAcquisition", "customerRetention",
               "customerEngagement", "worldMap")

  output$country_filter_sidebar <- renderUI({
    req(input$platform, input$sidebar_menu)
    if (!(input$sidebar_menu %in% vs_tabs)) return(NULL)

    countries <- tryCatch({
      if (!DBI::dbExistsTable(app_connection, "df_geo_sales_by_country")) return(NULL)
      DBI::dbGetQuery(app_connection, paste0(
        "SELECT DISTINCT ship_country FROM df_geo_sales_by_country",
        " WHERE platform_id = '", input$platform, "'",
    ...
    choices <- c("all" = "all", stats::setNames(countries, countries))
    div(class = "px-3 pb-2",
        tags$label(translate("Country"), class = "text-muted small mb-1 d-block"),
        selectInput("vs_country", label = NULL, choices = choices, selected = "all"))
  })
