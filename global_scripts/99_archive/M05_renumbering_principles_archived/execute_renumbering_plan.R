...
source("M05_renumbering_principles/M05_fn_renumber_principles.R")
...
rule_results <- M05_renumbering_principles$batch_renumber(rules_mapping)
...
system("cp -r M05_renumbering_principles/* M00_renumbering_principles/")

# Update the module main file
m05_main_file <- readLines("M00_renumbering_principles/M05_fn_renumber_principles.R")
m00_main_file <- gsub("M05_renumbering_principles", "M00_renumbering_principles", m05_main_file)
...
for (file in m00_files) {
  if (file.exists(file) && !dir.exists(file)) {
    ...
    updated_content <- gsub("M05_renumbering_principles", "M00_renumbering_principles", content)
...
if (file.exists(readme_path)) {
  ...
  updated_readme <- gsub("M05_renumbering_principles", "M00_renumbering_principles", readme_content)
...
cat("Module files updated. Original module is still intact at M05_renumbering_principles.\n")
...
issues <- M05_renumbering_principles$verify()
if (is.null(issues)) {
...
cat("3. Once verified, manually remove M05_renumbering_principles directory\n")