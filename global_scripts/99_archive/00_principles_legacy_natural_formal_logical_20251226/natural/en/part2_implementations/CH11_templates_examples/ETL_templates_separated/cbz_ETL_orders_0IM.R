...
main_execution <- function() {
  tryCatch({
    ...
    orders_processed <- process_orders_import(orders_raw_data, platform_code = "cbz")
    ...
    validation_result <- validate_orders_import(orders_standardized, platform_code = "cbz")
...
test_execution <- function() {
  tryCatch({
    ...
    required_fields <- c("order_id", "customer_id", "order_date", "order_status",
                         "total_amount", "currency", "platform_code", "import_timestamp")
