...
main_execution <- function() {
  tryCatch({
    ...
    sales_schema_transformed <- transform_sales_schema(sales_staged, platform_code = "{platform}")
    ...
    sales_enriched <- enrich_sales_data(sales_schema_transformed, platform_code = "{platform}")
    ...
    validation_result <- validate_sales_transform(sales_transformed, platform_code = "{platform}")
...
test_execution <- function() {
  ...
  required_mp102_fields <- c("order_id", "customer_id", "product_id", "quantity", "unit_price", "total_amount", "platform_code")
  ...
  platform_check <- dbGetQuery(transformed_data, sprintf("SELECT DISTINCT platform_code FROM %s", transformed_table_name))
