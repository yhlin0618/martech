...
main_execution <- function() {
  tryCatch({
    ...
    sales_processed <- process_sales_import(sales_combined, platform_code = "eby")
    ...
    validation_result <- validate_sales_import(sales_standardized, platform_code = "eby")
...
test_execution <- function() {
  tryCatch({
    ...
    if (length(platform_codes) != 1 || platform_codes[1] != "eby") {
