...
main_execution <- function() {
  tryCatch({
    ...
    sales_staged <- stage_sales_data(sales_encoded, platform_code = "{platform}")
    ...
    validation_result <- validate_sales_staging(sales_dated, platform_code = "{platform}")
...
test_execution <- function() {
  ...
  required_fields <- c("order_id", "customer_id", "product_id", "platform_code", "staging_timestamp")
  ...
  platform_check <- dbGetQuery(staged_data, sprintf("SELECT DISTINCT platform_code FROM %s", staged_table_name))
