...
SCHEMA_customer_dna <- list(
  ...
  columns = list(
    ...
    customer_id = list(
      type = "VARCHAR",
      required = TRUE,
      description = "Customer identifier (links to df_customer_profile)",
      example = "CUST_001234"
    ),
  ...
  constraints = list(
    ...
    foreign_keys = list(
      customer = "References df_customer_profile(customer_id)"
    )
  ),
...
validate_customer_tables <- function(con) {
  ...
  if (results$profile$valid && results$dna$valid) {
    orphan_check <- dbGetQuery(con, "
      ...
      WHERE NOT EXISTS (
        SELECT 1 FROM df_customer_profile p
