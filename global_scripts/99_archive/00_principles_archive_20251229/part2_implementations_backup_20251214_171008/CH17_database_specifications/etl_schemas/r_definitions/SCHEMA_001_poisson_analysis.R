...
SCHEMA_poisson_analysis <- list(
  ...
  columns = list(
    ...
    predictor_type = list(
      type = "VARCHAR",
      required = TRUE,
      description = "Category of predictor (e.g., 'time_feature', 'product_feature', 'price_feature')",
...
prepare_poisson_analysis_data <- function(model_results, product_line_id, platform) {
  ...
  df <- data.frame(
    ...
    predictor_type = classify_predictor(rownames(coef_summary)),
...
# Helper function to classify predictors
classify_predictor <- function(predictor_names) {
  case_when(
    predictor_names %in% c("year", "day", "month", "monday", "tuesday",
                          "wednesday", "thursday", "friday", "saturday", "sunday") ~ "time_feature",
    predictor_names %in% c("price", "discount", "original_price", "special_price") ~ "price_feature",
    predictor_names %in% c("brand", "manufacturer", "product_name") ~ "product_feature",
    TRUE ~ "other_feature"
  )
}