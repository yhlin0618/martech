...
- **Registry**: `etl_schemas/schema_registry.yaml`
- **Core Schemas**: `etl_schemas/core_schemas.yaml`
- **Extensions**: `etl_schemas/platform_extensions/`
...
platform_code <- "cbz"  # or "eby", "amz", etc.
etl_scripts <- list.files(
  path = "scripts/update_scripts",
  pattern = sprintf("^%s_ETL.*\\.R$", platform_code),
  full.names = TRUE,
  recursive = TRUE
)
```
...
sales:
  ...
  - platform_code   # Required (3-letter code)
...
transform_to_core_schema <- function(raw_data, platform_code) {
  # Map to core fields
  core_data <- raw_data %>%
    mutate(
  ...
  for (field in platform_fields) {
    prefixed_name <- paste0(platform_code, "_", field)
...
api_import_pattern <- function(api_endpoint, platform_code) {
  # 1. Fetch raw data
  raw_data <- fetch_from_api(api_endpoint)
  
  # 2. Transform to core schema
  core_data <- transform_to_core_schema(raw_data, platform_code)
  
  # 3. Validate
  validate_etl_schema(core_data, platform_code, "sales")
  
  # 4. Write to database
  table_name <- sprintf("df_%s_sales_raw", platform_code)
...
file_import_pattern <- function(file_path, platform_code) {
  # 1. Read file
...
handle_missing_fields <- function(raw_data, platform_code) {
...
```r
test_core_fields <- function(con, platform_code) {
  table_name <- sprintf("df_%s_sales_raw", platform_code)
...
```r
test_extension_prefix <- function(con, platform_code) {
  table_name <- sprintf("df_%s_sales_raw", platform_code)
    ...
    map_chr(platforms, function(p) {
      sprintf(
        "SELECT order_id, customer_id, product_id, 
                quantity, total_amount, platform_code
...
```r
# Compatibility view
create_compatibility_view <- function(con, platform_code) {
  query <- sprintf("
...
- **Core Schemas**: etl_schemas/core_schemas.yaml
- **Platform Extensions**: etl_schemas/platform_extensions/
