      ...
      if (!is.null(input$dna_files) && nrow(input$dna_files) > 0) {
        ...
        if (!is.null(fields$customer_id) && !is.null(fields$time) && !is.null(fields$amount)) {
          ...
          if (!exists("SKIP_SQL_UPLOAD") || !SKIP_SQL_UPLOAD) {
            ...
            tryCatch({
              # 先刪除舊資料
              if (DBI::dbExistsTable(con, "uploaded_sales")) {
                DBI::dbRemoveTable(con, "uploaded_sales")
              }
              # 寫入新資料
              DBI::dbWriteTable(con, "uploaded_sales", standardized_data)
            }, error = function(e) {
              print(paste("Error writing to uploaded_sales:", e$message))
            })
          ...
          if (!exists("SKIP_SQL_UPLOAD") || !SKIP_SQL_UPLOAD) {
            tryCatch({
              ...
              # 寫入新資料
              DBI::dbWriteTable(con, "uploaded_sales", combined_data)
            }, error = function(e) {
              print(paste("Error writing to uploaded_sales:", e$message))
            })
          }
