...
.InitEnv$detect_script_path <- function() {
  for (i in rev(seq_len(sys.nframe()))) {
    ...
    if (!is.null(f) && nzchar(f)) return(normalizePath(f))
  }
  ...
  if (length(fi) == 1) return(normalizePath(fi))
  ""
}

.InitEnv$get_mode <- function(path) {
  parent <- tolower(basename(dirname(path)))
  if (identical(parent, "update_scripts")) return("UPDATE_MODE")
  if (identical(parent, "global_scripts")) return("GLOBAL_MODE")
  "APP_MODE"
}
...
.InitEnv$autoinit <- function() {
  ...
  if (identical(.InitEnv$mode, .InitEnv$OPERATION_MODE)) return(invisible(NULL))
  ...
  if (!exists("APP_DIR", envir = .InitEnv)) {
    ...
    list2env(list(
      ...
      GLOBAL_DATA_DIR      = file.path(base, "scripts", "global_scripts", "global_data"),
      GLOBAL_PARAMETER_DIR = file.path(base, "scripts", "global_scripts", "global_data", "parameters"),
    ...
    # ---- 讀取 db_paths.R ------------------------------------------------------
    r_path <- file.path(.InitEnv$GLOBAL_DIR, "global_data", "db_paths.R")
    if (file.exists(r_path)) {
      sys.source(r_path, envir = .InitEnv)
      # 將資料庫路徑清單同步到全域環境
  ...
  init_files <- switch(
    OPERATION_MODE,
    UPDATE_MODE = c("sc_initialization_app_mode.R",
                    "sc_initialization_update_mode.R"),   # ← 兩支都跑
