...
# ── 初始化環境 ────────────────────────────────────────────────────────
# 載入必要的 global_scripts 函數
# 注意：不使用完整的 autoinit() 因為它會載入不需要的產品線設定
if (exists(".InitEnv") && exists("GLOBAL_DIR", envir = .InitEnv)) {
  # 如果 .Rprofile 已經設定了路徑，使用它
  global_dir <- .InitEnv$GLOBAL_DIR
} else {
  # 否則手動設定路徑
  global_dir <- file.path(getwd(), "scripts", "global_scripts")
}

# 只載入需要的函數目錄
if (dir.exists(file.path(global_dir, "02_db_utils", "tbl2"))) {
  source(file.path(global_dir, "02_db_utils", "tbl2", "fn_tbl2.R"))
}
...
# ── 系統初始化 ────────────────────────────────────────────────────────
# autoinit() 已經載入了必要的 global_scripts
# 現在只需要載入應用程式特定的配置
...
source("modules/module_dna_multi_basic.R")        # DNA 分析模組
source("modules/module_dna_multi_enhanced.R")     # 增強版 DNA 分析模組（含客戶明細）
source("modules/module_customer_segment.R")       # 客戶區隔模組
...
main_app_ui <- bs4DashPage(
  ...
  sidebar = bs4DashSidebar(
    ...
    div(class = "step-indicator",
        ...
        div(class = "step-item", id = "step_3", "3. 客戶區隔")
    ),
  ...
  body = bs4DashBody(
    ...
    bs4TabItems(
      ...
      bs4TabItem(
        tabName = "customer_segment",
        customerSegmentModuleUI("customer_segment1")
      ),
        ...
        fluidRow(
          bs4Card(
            ...
            div(
              ...
              p(strong("聯絡資訊: "), 
                tags$a(
                  href = "mailto:partners@peakedges.com",
                  "partners@peakedges.com",
...
server <- function(input, output, session) {
  ...
  observe({
    if (!is.null(input$sidebar_menu)) {
      ...
      } else if (input$sidebar_menu == "customer_segment") {
