...
uploadModuleUI <- function(id) {
  ...
  div(id = ns("step1_box"),
      ...
      div(
        ...
        fileInput(ns("excel_multiple"), "多檔案上傳 (自動合併)", multiple = TRUE, accept = c(".xlsx", ".xls", ".csv")),
        br()
      ),
      ...
      div(
        h5("1.2 上傳銷售資料 (選填)"),
        fileInput(ns("sales_multiple"), "多檔案上傳 (自動合併)", multiple = TRUE, accept = c(".xlsx", ".xls", ".csv")),
        br()
      ),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$load_btn, {
      ...
      if (!is.null(working_data())) {
        msg <- c(msg, sprintf("✅ 已上傳評論資料，共 %d 筆", nrow(working_data())))
      }
      if (!is.null(sales_data())) {
        msg <- c(msg, sprintf("✅ 已上傳銷售資料，共 %d 筆", nrow(sales_data())))
      }
      if (length(msg) == 0) {
        msg <- "⚠️ 請至少上傳評論資料"
      }
