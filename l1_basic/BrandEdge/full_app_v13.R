...
# ---- Step 2：產生屬性並評分 -------------------------------------------------
step2_ui <- hidden(div(id = "step2_box", h4("步驟 2：產生屬性並評分"), actionButton("gen_facets", "產生 6 個屬性", class = "btn-secondary"), verbatimTextOutput("facet_msg"), br(), sliderInput("nrows", "取前幾列 (評分)", min = 1, max = 100, value = 50, step = 1, ticks = FALSE), shinyjs::disabled(actionButton("score", "開始評分", class = "btn-primary")), br(),textOutput("eta"), br(), DTOutput("score_tbl"), br(), actionButton("to_step3", "下一步 ➡️", class = "btn-info"),includeMarkdown("md/contacts.md")))
...
server <- function(input, output, session) {
  ...
  observeEvent(input$score, {
    ...
    attrs <- facets_rv(); req(length(attrs) > 0)
    dat   <- head(working_data(), input$nrows)
    
    # ── 組成「一列一屬性」的工作清單 ----------------------------------------
    task_df <- tidyr::crossing(
      idx   = seq_len(nrow(dat)),
      attr  = attrs
    ) |>
      mutate(
        row_json = purrr::map_chr(
          idx, ~ toJSON(dat[.x, c("Variation", "Title", "Body")],
        ...
        prompt = purrr::pmap(
          ...
          \(a, js) list(
            list(role = "system", content = "你是一位數據分析師，請用繁體中文回答。"),
            list(role = "user",
                 content = sprintf("以下 JSON：\n%s\n\n請只回%s:<1-5或無>", js, a))
          )
        )
      )
    
    # ── 平行呼叫 GPT --------------------------------------------------------
    withProgress(message = "評分中 (平行)…", value = 0, {
      res_vec <- future_map_chr(
        task_df$prompt,
        \(msgs) {
    ...
    task_df$result <- res_vec
    task_df$value  <- purrr::map_dbl(task_df$result, safe_value)
    
    # ── 還原成寬格式 --------------------------------------------------------
    score_wide <- task_df |>
      select(idx, attr, value) |>
      tidyr::pivot_wider(names_from = attr, values_from = value) |>
      arrange(idx) |>
      mutate(Variation = dat$Variation) |>
      select(Variation, everything(), -idx)
    
    
    other_col <- dat %>% select(-c("Variation","Title","Body"))
    result_df <- bind_cols(other_col,score_wide)
    working_data(result_df)
    output$score_tbl <- renderDT(score_wide, selection = "none")
    ...
    dbExecute(
      ...
      params = list(
        ...
        toJSON(score_wide, dataframe = "rows", auto_unbox = TRUE)
      )
    )
    
    shinyjs::disable("score")   # 避免重複點
