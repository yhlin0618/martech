# =============================================================================
# Test Markdown App
# Purpose: Generate Markdown content using GPT and compile to PDF
# =============================================================================
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(6,
           ...
           actionButton("compile_pdf", "編譯 PDF", class = "btn-success"),
           actionButton("compile_html", "編譯 HTML", class = "btn-info"),
           hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$compile_pdf, {
    ...
    html_compilation_success <- FALSE
    
    # 嘗試 PDF 編譯
    tryCatch({
      cat("Trying PDF compilation with minimal settings...\n")
      rmarkdown::render(
        input = md_file,
      ...
      cat("PDF compilation failed:", e$message, "\n")
      compilation_success <- FALSE
    })
    
    # 如果 PDF 失敗，嘗試 HTML 編譯
    if (!compilation_success) {
      tryCatch({
        cat("PDF compilation failed, trying HTML compilation...\n")
        rmarkdown::render(
          input = md_file,
        ...
        if (file.exists(html_file)) {
          html_compilation_success <- TRUE
          rv$status <- paste("PDF 編譯失敗，但 HTML 編譯成功:", basename(html_file))
          rv$html_path <- html_file
        }
      }, error = function(e) {
        cat("HTML compilation also failed:", e$message, "\n")
        html_compilation_success <- FALSE
      })
    }
    
    # 如果都失敗了
    if (!compilation_success && !html_compilation_success) {
      rv$status <- "PDF 和 HTML 編譯都失敗了。請檢查 LaTeX 安裝或使用 HTML 格式。"
      # 檢查是否有 log 檔案
  ...
  observeEvent(input$compile_html, {
    ...
    tryCatch({
      ...
      if (file.exists(html_file)) {
        rv$status <- paste("HTML 編譯成功:", basename(html_file))
        rv$html_path <- html_file
