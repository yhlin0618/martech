...
| 指標 | 英文全名 | 定義 | 計算來源 | 單位 |
|------|---------|------|---------|------|
| **R** | Recency | 每位顧客的最後購買距今天數 | DNA 分析 | 天 |
| **F** | Frequency | 每位顧客的購買頻率（每位顧客的交易次數） | DNA 分析 | 次 |
| **M** | Monetary | 每位顧客的平均客單價 | DNA 分析 | 金額 |
| **IPT** | Inter-Purchase Time | 每位顧客的平均購買週期（兩次購買時間間隔） | DNA 分析 | 天 |
| **ni** | Number of Interactions | 每位顧客的總交易次數 | DNA 分析 | 次 |
| **CAI** | Customer Activity Index | 每位顧客的活躍度指數（基於購買間隔變化） | DNA 分析 | 0,1 |

### 3.2 衍生指標定義

| 指標 | 定義 | 計算公式 | 說明 |
|------|------|---------|------|
| **avg_ipt** | 全體平均購買週期 | median(ipt_value) | 用於所有顧客的購買時間周期長度 |
...
| 等級 | 定義條件 (ni ≥ 4) | 說明 |
|------|------------------|------|
| **高** | cai_ecdf ≥ 0.8 | 漸趨活躍戶 |
| **中** | 0.2 ≤ cai_ecdf < 0.8 | 穩定消費 |
| **低** | cai_ecdf < 0.2 | 漸趨靜止戶 |
| **未知** | NA | 無足夠數據 |

**計算策略**：
- **優先策略** (ni ≥ 4)：使用 CAI（反映活躍度變化趨勢）
- **排除** (ni < 4)：新客無法計算活躍度，設為 NA
...
**為什麼使用 CAI？**
- CAI 反映顧客未來的消費行為，是否會更活躍或趨於靜止（漸趨活躍、穩定消費或漸趨靜止）
...
**關鍵限制**: 只對 `ni >= 4` 的客戶計算 RFM 分數**(修改2：RFM分數，不用考慮ni <4的問題，因為他和CAI無關，可以直接計算)**
...
calculate_rfm_tags <- function(customer_data) {
  ...
  customer_data %>%
    mutate(
      ...
      tag_010_rfm_f = f_value,          # Frequency（次）
...
| **tag_010** | rfm_f | f_value | - | 所有客戶 | 購買頻率（次） |
| **tag_011** | rfm_m | m_value | - | 所有客戶 | 貨幣價值（金額） |
| **tag_012** | rfm_score | r_score + f_score + m_score | 3-15 | 所有客戶 | RFM 總分 |
...
|---------|---------|------|---------|------|
| **tag_017** | lifecycle_stage | 5種階段 | 所有客戶 | 顧客動態階段（中文） |
...
### 9.3 下次購買日期預測(修改3：考慮剩餘時間算法？如下圖所示)

**計算邏輯**：

[^1) 先用「剩餘時間」思維，而不是「今天 + 週期」]: 

![image-20251031223347875](C:\Users\utaipei\AppData\Roaming\Typora\typora-user-images\image-20251031223347875.png)

[^]: 

![image-20251031223602746](C:\Users\utaipei\AppData\Roaming\Typora\typora-user-images\image-20251031223602746.png)

![image-20251031223803713](C:\Users\utaipei\AppData\Roaming\Typora\typora-user-images\image-20251031223803713.png)
...
| tag_012 | RFM總分 | r_score + f_score + m_score | 3-15 |  | RFM |
...
| 標籤 | 名稱 | 值域 | 條件 | 模組 |
|------|------|------|------|------|
| tag_017 | 顧客動態 | 新客/主力客/睡眠客/半睡客/沉睡客 | 所有 | 狀態 |
...
| **R/F/M (RFM)** | Quantile | P20,P40,P60,P80 | - | 全體 | ✅ |
