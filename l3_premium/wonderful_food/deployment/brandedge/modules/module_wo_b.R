...
# 專用關鍵因素分析UI
keyFactorsModuleUI <- function(id) {
  ns <- NS(id)
  tagList(
    h5("關鍵成功因素識別"),
    textOutput(ns("key_factors")),
...
# 專用理想點分析UI
idealPointModuleUI <- function(id) {
  ns <- NS(id)
  tagList(
    h5("理想點基準設定"),
    textOutput(ns("ideal_definition")),
    br(),
    h5("品牌與理想點距離排名"),
    DTOutput(ns("ideal_rank"))
  )
...
# 專用關鍵因素分析服務器
keyFactorsModuleServer <- function(id, data, key_vars) {
  moduleServer(id, function(input, output, session) {
    ...
    output$factor_importance <- renderDT({
      ...
      importance_data <- df %>%
        ...
        pivot_longer(everything(),
                     ...
                     values_to = "value") %>%
        separate(metric_attr, c("attribute", "metric"), sep = "_") %>%
        pivot_wider(names_from = metric, values_from = value) %>%
        mutate(
          重要性指數 = round(平均分 * (1 + 標準差), 2),
          排名 = rank(-重要性指數)
        ) %>%
        select(屬性 = attribute, 平均分, 標準差, 重要性指數, 排名) %>%
        arrange(排名)
      
      DT::datatable(importance_data,
                    rownames = FALSE,
                    options = list(pageLength = 10, searching = FALSE)) %>%
        formatRound(columns = c("平均分", "標準差", "重要性指數"), digits = 2)
    })
  })
}

# 專用理想點分析服務器 - 採用MAMBA方式（超過理想點得分）
idealPointModuleServer <- function(id, data, key_vars) {
  moduleServer(id, function(input, output, session) {
    
    # 理想點定義說明 - 採用新的MAMBA公式
    output$ideal_definition <- renderText({
      "理想點 = 每個屬性的最高分。品牌在某屬性上達到或超過理想點即得1分"
    })
    ...
    output$ideal_rank <- renderDT({
      ...
      rank_data <- df %>%
        ...
        mutate(
          # 計算在每個關鍵屬性上是否達到理想點
          超過理想點數 = sum(c_across(all_of(keys)) >= ideal_points[keys], na.rm = TRUE),
          # 計算超過理想點的百分比
          理想點達成率 = round((超過理想點數 / length(keys)) * 100, 1),
          # 計算平均分作為參考
          平均分數 = mean(c_across(all_of(keys)), na.rm = TRUE)
        ) %>%
        ungroup() %>%
        # 添加詳細的屬性表現
        mutate(
          達標屬性 = apply(select(., all_of(keys)), 1, function(row) {
            達標 <- keys[row >= ideal_points[keys]]
            if(length(達標) > 0) {
              paste(head(達標, 5), collapse = "、")
            } else {
              "無"
            }
          }),
          未達標屬性 = apply(select(., all_of(keys)), 1, function(row) {
            未達標 <- keys[row < ideal_points[keys]]
            if(length(未達標) > 0) {
              paste(head(未達標, 3), collapse = "、")
            } else {
              "全部達標"
            }
          })
        ...
        arrange(desc(超過理想點數), desc(平均分數))
      
      DT::datatable(rank_data,
                    caption = paste0("品牌理想點分析 - 共", length(keys), "個關鍵屬性"),
                    rownames = FALSE,
                    options = list(
                      pageLength = 10, 
                      searching = TRUE,
                      columnDefs = list(
                        ...
                        list(width = '30%', targets = 5)
                      )
                    )) %>%
        formatRound(columns = c("平均分數"), digits = 2) %>%
        formatStyle(
          "超過理想點數",
          backgroundColor = styleInterval(
            c(length(keys)*0.3, length(keys)*0.6),
