...
# ── 添加信息圖標提示 ────────────────────────────────────────────────────────
add_info_icon <- function(var_id, hints_df = NULL, size = "sm") {
  hint_text <- get_hint(var_id, hints_df)
...
# ── 批次添加提示 ────────────────────────────────────────────────────────────
add_hints_batch <- function(ui_list, var_ids, hints_df = NULL, enable_hints = TRUE) {
  if (!enable_hints || length(ui_list) != length(var_ids)) {
    return(ui_list)
  }
  
  Map(function(ui, id) {
    add_hint(ui, id, hints_df, enable_hints)
  }, ui_list, var_ids)
}

# ── 創建帶提示的標籤 ────────────────────────────────────────────────────────
create_label_with_hint <- function(label_text, var_id, hints_df = NULL) {
  hint_icon <- add_info_icon(var_id, hints_df)
