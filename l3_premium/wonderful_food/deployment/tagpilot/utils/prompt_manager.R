...
# ── 快速建立單一 Prompt ─────────────────────────────────────────────────────
quick_prompt <- function(var_id, ..., prompts_df = NULL) {
  variables <- list(...)
  prepare_gpt_messages(var_id, variables, prompts_df)
}

# ── 批次準備多個 Prompts ────────────────────────────────────────────────────
prepare_batch_prompts <- function(var_ids, variables_list, prompts_df = NULL) {
  if (length(var_ids) != length(variables_list)) {
    stop("var_ids and variables_list must have the same length")
  }
  
  Map(function(id, vars) {
    prepare_gpt_messages(id, vars, prompts_df)
  }, var_ids, variables_list)
}

# ── 驗證 Prompt 變數 ────────────────────────────────────────────────────────
validate_prompt_variables <- function(var_id, prompts_df = NULL) {
  raw_prompt <- get_prompt(var_id, prompts_df)
...
save_new_prompt <- function(analysis_name, var_id, prompt_text, 
  ...
  # 載入現有 prompts
  existing_prompts <- load_prompts(prompt_file)
...
# ── 格式化 Prompt 顯示 ──────────────────────────────────────────────────────
format_prompt_display <- function(var_id, variables = list(), prompts_df = NULL) {
