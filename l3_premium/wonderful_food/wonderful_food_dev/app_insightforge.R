...
# ── 系統初始化 ────────────────────────────────────────────────────────────
source("config/insightforge/packages.R")    # 載入套件管理
source("config/insightforge/config.R")      # 載入配置設定（包含 .env 載入）

# 載入模組配置（如果存在）
if (file.exists("config/insightforge/module_config.R")) {
  source("config/insightforge/module_config.R")  # 載入模組選擇配置
}

# 載入工具函數 (shared 版本)
if (file.exists("utils/shared/openai_utils.R")) {
  source("utils/shared/openai_utils.R")  # 提供 chat_api 函數
}
if (file.exists("utils/shared/hint_system.R")) {
  source("utils/shared/hint_system.R")
}
if (file.exists("utils/shared/prompt_manager.R")) {
  source("utils/shared/prompt_manager.R")
}
...
source("utils/shared/data_access.R")  # 資料存取工具（整合 tbl2）- shared 版本

# 載入改進版模組（如果存在），否則載入原版
if (file.exists("modules/insightforge/module_wo_b_v2.R")) {
  source("modules/insightforge/module_wo_b_v2.R")  # 使用改進版分析模組
} else {
  source("modules/shared/module_wo_b.R")     # 原版分析模組 (shared)
}
...
if (use_complete_score && file.exists("modules/insightforge/module_upload_complete_score.R")) {
  source("modules/insightforge/module_upload_complete_score.R")  # 新的已評分資料上傳模組
  source("modules/insightforge/module_score_v2.R")   # 評分模組（UI 仍需要 scoreModuleUI）
  cat("✅ 使用已評分資料上傳模組\n")
  USE_COMPLETE_SCORE <- TRUE
} else {
  source("modules/insightforge/module_upload.R")     # 原始上傳模組
  source("modules/insightforge/module_score_v2.R")   # 評分模組（已整合 prompt 管理）
  cat("📝 使用原始上傳與評分模組\n")
  USE_COMPLETE_SCORE <- FALSE
}

# 載入新增的行銷策略模組
if (file.exists("modules/insightforge/module_keyword_ads.R")) {
  source("modules/insightforge/module_keyword_ads.R")  # 關鍵字廣告模組
}
if (file.exists("modules/insightforge/module_product_dev.R")) {
  source("modules/insightforge/module_product_dev.R")  # 新品開發模組
}
