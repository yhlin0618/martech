...
# 載入配置模組（包含 .env 載入）
source("config/brandedge/config.R")
...
# ── 載入模組 ────────────────────────────────────────────────────────────────
# Shared 模組
source("modules/brandedge/module_wo_b.R")

# BrandEdge 專用模組
source("modules/brandedge/module_brandedge_flagship.R")

# 載入增強版市場輪廓分析模組
if (file.exists("modules/brandedge/module_market_profile_enhanced.R")) {
  source("modules/brandedge/module_market_profile_enhanced.R")
}

# 使用 Supabase 登入模組替代原有登入模組
# if (file.exists("modules/brandedge/module_login.R")) {
#   source("modules/brandedge/module_login.R")
# }

# 載入完整評分上傳模組
if (file.exists("modules/brandedge/module_upload_complete_score.R")) {
  source("modules/brandedge/module_upload_complete_score.R")
}
