...
if (file.exists("modules/module_wo_b_v2.R")) {
  ...
  source("modules/module_wo_b.R")     # 原版分析模組
}
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    mean_data <- score_mod$scored_data() %>%
  ...
  observeEvent(input$run_poisson, {
    req(score_mod$scored_data(), sales_data())
    ...
    tryCatch({
      ...
      if (length(common_vars) > 0) {
        ...
        if (length(eval_variations) == 0 || length(sales_variations) == 0) {
          showNotification("❌ 評分或銷售資料為空", type = "error")
          return()
        }
