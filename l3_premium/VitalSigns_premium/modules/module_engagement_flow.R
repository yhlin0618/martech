      ...
      column(6,
        bs4Card(
          ...
          div(
            style = "padding: 10px; background: #f8f9fa; border-radius: 5px; margin-top: 10px;",
            h6("📊 圖表解讀說明", style = "color: #2c3e50; font-weight: bold;"),
            p("散佈圖展示每個客戶的購買頻率與週期關係：", style = "margin-bottom: 5px;"),
            tags$ul(
              tags$li("橫軸：購買頻率（總購買次數）"),
              tags$li("縱軸：平均購買週期（兩次購買間隔天數）"),
              tags$li("點的大小：總消費金額"),
              tags$li("顏色：客戶價值分群")
            ),
            p("理想客戶位於右下角：高頻率、短週期、大消費", style = "font-style: italic; color: #666;")
          )
        )
      ),
      ...
      if(nrow(pattern_data) > 0) {
        ...
        pattern_data <- pattern_data %>%
          mutate(
            ...
            hover_text = paste0(
              "客戶ID: ", customer_id, "<br>",
              "購買頻率: ", times, "次<br>",
              "平均週期: ", round(avg_cycle, 1), "天<br>",
              "總消費: $", format(round(total_spent, 0), big.mark = ","), "<br>",
              "狀態: ", nes_status
            ),
            # 為顏色分組
            value_group = case_when(
              total_spent >= quantile(total_spent, 0.75, na.rm = TRUE) ~ "高價值",
              total_spent >= quantile(total_spent, 0.50, na.rm = TRUE) ~ "中高價值",
              total_spent >= quantile(total_spent, 0.25, na.rm = TRUE) ~ "中低價值",
              TRUE ~ "低價值"
            )
          ) %>%
        ...
        if(nrow(pattern_data) > 0) {
          ...
          plot_ly(pattern_data,
            ...
            layout(
              title = list(text = "購買週期頻率散佈圖", font = list(size = 14)),
              xaxis = list(
                title = "購買頻率（次數）",
                zeroline = TRUE,
                gridcolor = 'rgba(0,0,0,0.1)',
