...
source("utils/ai_analysis_manager.R")  # AI分析管理模組
...
dnaMultiModuleUI <- function(id, enable_hints = TRUE) {
  ...
  div(
    ...
    div(
      ...
      h5("📋 操作說明", style = "color: #007bff; margin-bottom: 10px;"),
      tags$ol(
        style = "margin-bottom: 0; padding-left: 20px;",
        tags$li(HTML("<b>步驟一：</b>點選你要分析的顧客標籤，例如，你可以點選購買金額。")),
        tags$li(HTML("<b>步驟二：</b>點選你的分析工具，例如，你可以點選執行客戶分群。")),
        tags$li(HTML("<b>步驟三：</b>點選檢視選項，例如，你可以點選統計圖表。"))
      ),
      p("重複上述步驟，假設您想查看顧客最近來店時間，則可重複上述步驟檢視分析結果和AI行銷建議。", 
      ...
      fluidRow(
        ...
        column(3,
          wellPanel(
            ...
            actionButton(ns("show_ipt"), "⏱️ 購買週期", class = "btn-outline-info btn-block", style = "margin-bottom: 10px;"),
            ...
            actionButton(ns("show_cri"), "🎯 顧客穩定度指標", class = "btn-outline-info btn-block", style = "margin-bottom: 10px;"),
            ...
            wellPanel(
              ...
              fluidRow(
                column(12,
                  ...
                  plotlyOutput(ns("plot_hist"), height = "400px")
                )
              ),
              fluidRow(
                column(6,
                  div(
                    style = "background: #e8f4f8; padding: 15px; border-radius: 5px; margin-top: 15px; font-size: 14px;",
                    h6("📊 統計摘要", style = "font-weight: bold; margin-bottom: 10px;"),
                    uiOutput(ns("stat_summary"))
                  )
                ),
                column(6,
                  div(
                    style = "background: #fff3cd; padding: 15px; border-radius: 5px; margin-top: 15px; font-size: 14px;",
                    h6("💡 分布洞察", style = "font-weight: bold; margin-bottom: 10px;"),
...
dnaMultiModuleServer <- function(id, con, user_info, uploaded_dna_data = NULL, chat_api = NULL, enable_hints = TRUE, enable_prompts = TRUE) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      if (!is.null(uploaded_dna_data) && !is.null(uploaded_dna_data())) {
        ...
        values$status_text <- paste("✅ 已從步驟1載入資料，共", nrow(uploaded_dna_data()), "筆記錄，", 
                                   length(unique(uploaded_dna_data()$customer_id)), "位客戶。正在自動進行DNA分析...")
    ...
    observeEvent(input$run_ai_analysis, {
      ...
      # 定義分析函數
      analysis_function <- function(data, metric) {
        # 生成AI分析結果
        ai_summary <- NULL
        ai_recommendations <- NULL
        ...
        tryCatch({
          ...
          # 將 HTML 格式的摘要轉換為純文字或 markdown
          formatted_summary <- if (!is.null(ai_summary)) {
            # 保留 HTML 格式，AI 管理器會處理
            ai_summary
          } else {
            NULL
          }
          
          # 保持分群結構以便AI管理器可以結構化顯示
          formatted_recommendations <- if (!is.null(ai_recommendations) && length(ai_recommendations) > 0) {
            # 直接返回分群結構的建議
            ai_recommendations
          } else {
            NULL
          ...
          showNotification(paste("AI 分析錯誤:", e$message), type = "error")
          list(
            summary = paste("分析錯誤:", e$message),
      ...
      result <- execute_ai_analysis(
        ...
        force_refresh = TRUE,  # 強制重新分析
        analysis_function = analysis_function
      )
      
      # 更新值
      if (!is.null(result)) {
        values$ai_analysis_summary <- result$summary
        values$ai_recommendations <- result$recommendations
    ...
    generateAIAnalysisSummary <- function() {
      ...
      # 計算關鍵洞察
      high_value_pct <- round(segment_stats$count[segment_stats$segment == "高價值顧客"] / sum(segment_stats$count) * 100, 1)
      high_value_revenue_pct <- round(segment_stats$total_revenue[segment_stats$segment == "高價值顧客"] / sum(segment_stats$total_revenue) * 100, 1)
      
      summary_html <- paste0(
        "<div style='padding: 15px;'>",
        "<h5>📊 ", metric_name, " 分析重點</h5>",
        "<p style='color: #666; font-size: 14px;'>",
        "<b>分析時間：</b>", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "<br>",
        "<b>總客戶數：</b>", format(nrow(values$segmented_data), big.mark = ","), " 人",
        "</p>",
        "<br>",
        "<div style='background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #007bff;'>",
        "<h6>💡 關鍵洞察</h6>",
        "<ul style='margin-bottom: 0;'>",
        "<li><b>80/20法則驗證：</b>高價值顧客（", high_value_pct, "%）貢獻了 ", high_value_revenue_pct, "% 的營收</li>",
        "<li><b>指標分析焦點：</b>透過 ", metric_name, " 指標，識別出 ", nrow(segment_stats), " 個不同客群</li>",
        "<li><b>策略建議：</b>請參考下方的分群行銷建議，制定差異化策略</li>",
        "</ul>",
        "</div>",
        "</div>"
      )
    ...
    output$download_customer_details <- downloadHandler(
      ...
      content = function(file) {
        ...
        # 更新客群分組欄位名稱
        if(segment_col != "客群分組" && segment_col %in% names(display_data)) {
          display_data$客群分組 <- display_data[[segment_col]]
        }
        
        # 選擇要顯示的欄位（與畫面上顯示的相同）
        display_cols <- c("客戶ID", "客群分組")
        
        # 根據不同指標增加對應的欄位
        if(metric == "R") {
          if("最近購買(天)" %in% names(display_data)) display_cols <- c(display_cols, "最近購買(天)")
        } else if(metric == "F") {
          if("購買頻率(次)" %in% names(display_data)) {
            display_cols <- c(display_cols, "購買頻率(次)")
          } else if("購買次數" %in% names(display_data)) {
            display_cols <- c(display_cols, "購買次數")
          }
        } else if(metric == "M") {
          if("平均金額($)" %in% names(display_data)) display_cols <- c(display_cols, "平均金額($)")
        } else if(metric == "IPT") {
          if("購買週期(天)" %in% names(display_data)) display_cols <- c(display_cols, "購買週期(天)")
        } else if(metric == "CAI") {
          if("活躍度指數" %in% names(display_data)) display_cols <- c(display_cols, "活躍度指數")
        } else if(metric == "PCV") {
          ...
          if("顧客穩定度" %in% names(display_data)) {
            display_cols <- c(display_cols, "顧客穩定度")
          } else if("參與度分數" %in% names(display_data)) {
            display_cols <- c(display_cols, "參與度分數")
          }
        } else if(metric == "NES") {
        ...
        "IPT" = {
          if("ipt_mean" %in% names(data)) {
            ...
            paste0(
              ...
              "<br>",
              "• <b>定義：</b>平均購買間隔天數<br>",
              "• <b>平均數：</b>", round(ipt_mean, 1), " 天<br>",
              "• <b>中位數：</b>", round(ipt_median, 1), " 天<br>",
              "• <b>25%/75%分位數：</b>", round(ipt_q25, 1), " / ", round(ipt_q75, 1), " 天<br>",
              "• <b>變異數：</b>", round(ipt_var, 2), "<br>",
              "• <b>整體輪廓：</b>",
              if(ipt_var > (ipt_mean^2 * 0.5)) "購買週期差異大需分群管理" else "購買週期較為一致",
              "<br><br>",
              "📊 <b>週期分組：</b><br>",
              "• 短週期客戶(≤", round(ipt_q20, 1), "天)：", short_cycle, "人<br>",
              "• 中週期客戶：", mid_cycle, "人<br>",
              "• 長週期客戶(≮", round(ipt_q80, 1), "天)：", long_cycle, "人"
            )
          } else {
            "⏱️ <b>購買週期分析</b><br>• 無購買週期數據"
          }
        },
        "CAI" = {
        ...
        "CRI" = {
          if("cri" %in% names(data)) {
            ...
            paste0(
              ...
              "<br>",
              "• <b>定義：</b>顧客交易穩定度綜合評分<br>",
              "• <b>計算方式：</b>RFM加權分數 (0.3×R + 0.3×F + 0.4×M) 標準化至0-100分<br>",
              "• <b>管理意涵：</b>識別穩定貢獻客群，採用80/20法則優化資源配置<br><br>",
              "📊 <b>統計摘要：</b><br>",
              ...
              "• <b>整體輪廓：</b>",
              if(cri_var > 0.01) "穩定度差異大，需差異化服務策略" else "穩定度較為平均",
              "<br><br>",
              "📊 <b>80/20法則分組：</b><br>",
              "• 高穩定(Top 20%, ≥", round(cri_q80 * 100, 1), "分)：", high_value, "人<br>",
              "• 中穩定(Middle 60%)：", total - high_value - low_value, "人<br>",
              "• 低穩定(Bottom 20%, ≤", round(cri_q20 * 100, 1), "分)：", low_value, "人<br><br>",
              "💡 <b>穩定度層級解讀：</b><br>",
              "• Top 20%：VIP穩定客群，提供專屬服務維持忠誠<br>",
              "• Middle 60%：一般穩定客群，定期關懷提升價值<br>",
              "• Bottom 20%：不穩定風險客群，需積極挽留策略"
            )
          } else {
            "🎯 <b>顧客穩定度指標</b><br>• 資料不足"
          }
    ...
    output$stat_summary <- renderUI({
      ...
      insight <- switch(metric,
        "M" = {
          ...
          HTML(paste0(
            "• <b>平均數：</b>$", round(mean_val, 2), "<br>",
            "<span style='color: #666; font-size: 12px;'>　（顧客平均購買金額）</span><br>",
            "• <b>中位數：</b>$", round(q50, 2), "<br>",
            "<span style='color: #666; font-size: 12px;'>　（一半顧客購買金額高於此值）</span><br>",
            "• <b>第25百分位數：</b>$", round(q25, 2), "<br>",
            "<span style='color: #666; font-size: 12px;'>　（25%顧客購買金額低於此值）</span><br>",
            "• <b>第75百分位數：</b>$", round(q75, 2), "<br>",
            "<span style='color: #666; font-size: 12px;'>　（75%顧客購買金額低於此值）</span><br>",
            "• <b>變異程度：</b>", 
            if(var_val > (mean_val^2 * 0.5)) "顧客之間的購買金額差異很大" else "顧客之間的購買金額差異較小",
            "<br><br>",
            "🎯 <b>AI行銷建議：</b><br>",
        ...
        "R" = {
          ...
          HTML(paste0(
            "• <b>平均數：</b>", round(mean_val, 0), " 天<br>",
            "<span style='color: #666; font-size: 12px;'>　（顧客平均距離上次購買天數）</span><br>",
            "• <b>中位數：</b>", round(q50, 0), " 天<br>",
            "<span style='color: #666; font-size: 12px;'>　（一半顧客的最近購買時間）</span><br>",
            "• <b>第25百分位數：</b>", round(q25, 0), " 天<br>",
            "<span style='color: #666; font-size: 12px;'>　（25%顧客在此天數內購買）</span><br>",
            "• <b>第75百分位數：</b>", round(q75, 0), " 天<br>",
            "<span style='color: #666; font-size: 12px;'>　（75%顧客在此天數內購買）</span><br>",
            "• <b>變異程度：</b>",
            if(var_val > (mean_val^2 * 0.5)) "顧客之間的購買時間差異很大" else "顧客之間的購買時間差異較小",
            "<br><br>",
            "🎯 <b>AI行銷建議：</b><br>",
        ...
        "F" = {
          ...
          HTML(paste0(
            "• <b>平均數：</b>", round(mean_val, 1), " 次<br>",
            "<span style='color: #666; font-size: 12px;'>　（顧客平均購買次數）</span><br>",
            "• <b>中位數：</b>", round(q50, 1), " 次<br>",
            "<span style='color: #666; font-size: 12px;'>　（一半顧客購買次數高於此值）</span><br>",
            "• <b>第25百分位數：</b>", round(q25, 1), " 次<br>",
            "<span style='color: #666; font-size: 12px;'>　（25%顧客購買次數低於此值）</span><br>",
            "• <b>第75百分位數：</b>", round(q75, 1), " 次<br>",
            "<span style='color: #666; font-size: 12px;'>　（75%顧客購買次數低於此值）</span><br>",
            "• <b>變異程度：</b>",
            if(var_val > (mean_val^2 * 0.5)) "顧客之間的購買頻率差異很大" else "顧客之間的購買頻率差異較小",
            "<br><br>",
            "🎯 <b>AI行銷建議：</b><br>",
        ...
        "IPT" = {
          ...
          if(length(col_data) > 0) {
            ...
            HTML(paste0(
              "• <b>平均數：</b>", round(mean_val, 1), " 天<br>",
              "<span style='color: #666; font-size: 12px;'>　（顧客平均購買間隔天數）</span><br>",
              "• <b>中位數：</b>", round(q50, 1), " 天<br>",
              "<span style='color: #666; font-size: 12px;'>　（一半顧客購買週期短於此值）</span><br>",
              "• <b>第25百分位數：</b>", round(q25, 1), " 天<br>",
              "<span style='color: #666; font-size: 12px;'>　（25%顧客購買週期短於此值）</span><br>",
              "• <b>第75百分位數：</b>", round(q75, 1), " 天<br>",
              "<span style='color: #666; font-size: 12px;'>　（75%顧客購買週期短於此值）</span><br>",
              "• <b>變異程度：</b>",
              if(var_val > (mean_val^2 * 0.5)) "顧客之間的購買週期差異很大" else "顧客之間的購買週期差異較小",
              "<br><br>",
              "🎯 <b>AI行銷建議：</b><br>",
              "• 短週期客戶(≤", round(q20, 1), "天)：常規提醒<br>",
              "• 中週期客戶：定期推送<br>",
              "• 長週期客戶(≥", round(q80, 1), "天)：特殊激勵"
            ))
          } else {
            HTML("• 無購買週期數據可用")
          }
        ...
        "CAI" = {
          ...
          if(length(col_data) > 0) {
            ...
            HTML(paste0(
              "• <b>平均數：</b>", round(mean_val, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（顧客平均活躍度指數）</span><br>",
              "• <b>中位數：</b>", round(q50, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（一半顧客活躍度高於此值）</span><br>",
              "• <b>第25百分位數：</b>", round(q25, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（25%顧客活躍度低於此值）</span><br>",
              "• <b>第75百分位數：</b>", round(q75, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（75%顧客活躍度低於此值）</span><br>",
              "• <b>變異程度：</b>",
              if(var_val > (mean_val^2 * 0.5)) "顧客之間的活躍度差異很大" else "顧客之間的活躍度差異較小",
              "<br><br>",
              "🎯 <b>AI行銷建議：</b><br>",
              "• 高活躍客戶(≥", round(q80, 2), ")：維持策略<br>",
              "• 中活躍客戶：激勵計劃<br>",
              "• 低活躍客戶(≤", round(q20, 2), ")：喚醒策略"
            ))
          } else {
            HTML("• 無CAI數據可用")
          }
        ...
        "PCV" = {
          ...
          if(length(col_data) > 0) {
            ...
            HTML(paste0(
              "• <b>平均數：</b>$", round(mean_val, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（顧客平均過去消費總額）</span><br>",
              "• <b>中位數：</b>$", round(q50, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（一半顧客過去消費高於此值）</span><br>",
              "• <b>第25百分位數：</b>$", round(q25, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（25%顧客過去消費低於此值）</span><br>",
              "• <b>第75百分位數：</b>$", round(q75, 2), "<br>",
              "<span style='color: #666; font-size: 12px;'>　（75%顧客過去消費低於此值）</span><br>",
              "• <b>變異程度：</b>",
              if(var_val > (mean_val^2 * 0.5)) "顧客之間的過去消費差異很大" else "顧客之間的過去消費差異較小",
              "<br><br>",
              "🎯 <b>AI行銷建議：</b><br>",
              "• 高貢獻客戶(≥$", round(q80, 2), ")：專屬優惠<br>",
              "• 中貢獻客戶：提升方案<br>",
              "• 低貢獻客戶(≤$", round(q20, 2), ")：培養策略"
            ))
          } else {
            HTML("• 無PCV數據可用")
          }
        ...
        "CRI" = {
          ...
          if(length(col_data) > 0) {
            ...
            HTML(paste0(
              "• <b>平均數：</b>", round(mean_val * 100, 1), " 分<br>",
              "<span style='color: #666; font-size: 12px;'>　（顧客平均穩定度分數）</span><br>",
              "• <b>中位數：</b>", round(q50 * 100, 1), " 分<br>",
              "<span style='color: #666; font-size: 12px;'>　（一半顧客穩定度高於此值）</span><br>",
              "• <b>第25百分位數：</b>", round(q25 * 100, 1), " 分<br>",
              "<span style='color: #666; font-size: 12px;'>　（25%顧客穩定度低於此值）</span><br>",
              "• <b>第75百分位數：</b>", round(q75 * 100, 1), " 分<br>",
              "<span style='color: #666; font-size: 12px;'>　（75%顧客穩定度低於此值）</span><br>",
              "• <b>變異程度：</b>",
              if(var_val > (mean_val^2 * 0.5)) "顧客之間的穩定度差異很大" else "顧客之間的穩定度差異較小",
              "<br><br>",
              "🎯 <b>AI行銷建議（80/20法則）：</b><br>",
              "• 高穩定Top 20%(≥", round(q80 * 100, 1), "分)：VIP維護策略<br>",
              "• 中穩定Middle 60%：忠誠度提升計劃<br>",
              "• 低穩定Bottom 20%(≤", round(q20 * 100, 1), "分)：挽留激活方案"
            ))
          } else {
            HTML("• 無CRI數據可用")
          }
        ...
        "NES" = {
          if("nes_status" %in% names(data)) {
            ...
            HTML(paste0(
              "<b>顧客狀態分布：</b><br>",
              if("N" %in% names(nes_counts)) paste0("• 新客戶(N)：", nes_counts["N"], "人 (", round(nes_counts["N"]/total*100, 1), "%)<br>") else "",
              if("E0" %in% names(nes_counts)) paste0("• 主力客戶(E0)：", nes_counts["E0"], "人 (", round(nes_counts["E0"]/total*100, 1), "%)<br>") else "",
              if("S1" %in% names(nes_counts)) paste0("• 瞌睡客戶(S1)：", nes_counts["S1"], "人 (", round(nes_counts["S1"]/total*100, 1), "%)<br>") else "",
              if("S2" %in% names(nes_counts)) paste0("• 半睡客戶(S2)：", nes_counts["S2"], "人 (", round(nes_counts["S2"]/total*100, 1), "%)<br>") else "",
              if("S3" %in% names(nes_counts)) paste0("• 沉睡客戶(S3)：", nes_counts["S3"], "人 (", round(nes_counts["S3"]/total*100, 1), "%)<br><br>") else "",
              "🎯 <b>AI行銷建議：</b><br>",
              "• 新客戶：新手導引方案<br>",
              "• 主力客戶：VIP禮遇計劃<br>",
              "• 瞌睡/半睡客戶：喚醒優惠<br>",
      ...
      insight <- switch(metric,
        ...
        "IPT" = {
          if("ipt_mean" %in% names(data)) {
            ...
            HTML(paste0(
              "📊 <b>分布洞察：</b><br>",
              "• ≤30天週期：", short, " 位 (", round(short/length(col_data)*100, 1), "%)<br>",
              "• 31-90天週期：", medium, " 位 (", round(medium/length(col_data)*100, 1), "%)<br>",
              "• >90天週期：", long, " 位 (", round(long/length(col_data)*100, 1), "%)<br>",
              "• ", ifelse(var(col_data) > (mean(col_data)^2 * 0.5), "週期差異大", "週期相對穩定")
            ))
          } else {
            HTML("📊 <b>分布洞察：</b><br>• 無購買週期數據")
          }
        },
        "CAI" = {
          if("cai_value" %in% names(data) || "cai" %in% names(data)) {
            ...
            HTML(paste0(
              "📊 <b>分布洞察：</b><br>",
              "• 漸趨靜止(≤0.1)：", inactive, " 位 (", round(inactive/length(col_data)*100, 1), "%)<br>",
              "• 穩定消費(0.1-0.9)：", stable, " 位 (", round(stable/length(col_data)*100, 1), "%)<br>",
              "• 漸趨活躍(>0.9)：", active, " 位 (", round(active/length(col_data)*100, 1), "%)<br>",
              "• ", ifelse(active > inactive, "客群活躍度佳", "需提升活躍度")
            ))
          } else {
            HTML("📊 <b>分布洞察：</b><br>• 無活躍度數據")
          }
        },
        "PCV" = {
          if("pcv" %in% names(data) || "total_spent" %in% names(data)) {
            ...
            HTML(paste0(
              "📊 <b>分布洞察：</b><br>",
              "• 低價值(≤$", round(q20, 0), ")：", low_value, " 位 (20%)<br>",
              "• 中價值：", mid_value, " 位 (60%)<br>",
              "• 高價值(≥$", round(q80, 0), ")：", high_value, " 位 (20%)<br>",
              "• 80/20法則：20%客戶貢獻", round(sum(col_data[col_data >= q80])/sum(col_data)*100, 1), "%營收"
            ))
          } else {
            HTML("📊 <b>分布洞察：</b><br>• 無過去價值數據")
          }
        },
        "CRI" = {
          if("cri" %in% names(data)) {
            ...
            HTML(paste0(
              "📊 <b>分布洞察（80/20法則）：</b><br>",
              "• 低穩定(Bottom 20%)：", low_stability, " 位<br>",
              "• 中穩定(Middle 60%)：", mid_stability, " 位<br>",
              "• 高穩定(Top 20%)：", high_stability, " 位<br>",
              if(!is.na(revenue_ratio)) {
                paste0("• <b>80/20驗證：</b>Top 20%客戶貢獻", revenue_ratio, "%營收<br>")
              } else "",
              "• <b>策略建議：</b>",
              ifelse(high_stability/low_stability > 1, "穩定客群結構健康", "需強化客戶穩定度")
            ))
          } else {
            HTML("📊 <b>分布洞察：</b><br>• 無穩定度數據")
          }
        },
        "NES" = {
          nes_table <- table(data$nes_status)
    ...
    performSegmentation <- function() {
      ...
      if(metric_col %in% names(data) && metric != "NES") {
        ...
        # 創建統一的中文欄位表格
        values$temp_chinese_table <- convert_to_chinese_columns(data)
      ...
      if("ipt_mean" %in% names(data) && !all(is.na(data$ipt_mean))) {
        ...
        if(length(unique(ipt_values)) > 2) {
          ...
          if(ipt_quantiles[1] == ipt_quantiles[2]) {
            # 如果分位數相同，使用平均值分組
            ipt_mean <- mean(ipt_values)
            data$segment_ipt <- ifelse(data$ipt_mean <= ipt_mean, "短週期", "長週期")
            data$segment_ipt <- factor(data$segment_ipt, levels = c("短週期", "長週期"))
          } else {
            # 注意：IPT越小代表購買頻繁，所以標籤順序與數值相反
            data$segment_ipt <- cut(data$ipt_mean,
                                   breaks = c(-Inf, ipt_quantiles[1], ipt_quantiles[2], Inf),
                                   labels = c("短週期", "中週期", "長週期"),
                                   include.lowest = TRUE)
          }
        } else {
          # 值太少，全部歸為一組
          data$segment_ipt <- factor("中週期", levels = c("短週期", "中週期", "長週期"))
        }
      }
      ...
      if("cri" %in% names(data) && !all(is.na(data$cri))) {
        ...
        if(length(unique(cri_values)) > 2) {
          ...
          if(cri_quantiles[1] == cri_quantiles[2]) {
            # 如果分位數相同，使用平均值分組
            cri_mean <- mean(cri_values)
            data$segment_cri <- ifelse(data$cri <= cri_mean, "低穩定", "高穩定")
            data$segment_cri <- factor(data$segment_cri, levels = c("低穩定", "高穩定"))
          } else {
            data$segment_cri <- cut(data$cri,
                                    breaks = c(-Inf, cri_quantiles[1], cri_quantiles[2], Inf),
                                    labels = c("低穩定(20%)", "中穩定(60%)", "高穩定(20%)"),
                                    include.lowest = TRUE)
          }
        } else {
          # 值太少，全部歸為一組
          data$segment_cri <- factor("中穩定(60%)", levels = c("低穩定(20%)", "中穩定(60%)", "高穩定(20%)"))
        }
      }
    ...
    output$segmentation_view <- renderUI({
      ...
      tagList(
        ...
        div(
          ...
          downloadButton(ns("download_customer_details"), "📥 下載完整客戶名單", 
    ...
    output$ai_analysis_results <- renderUI({
      # 使用 AI 管理器呈現結果
      render_ai_analysis_ui(ai_manager, values$current_metric)
    })
    
    # 舊的備份程式碼（可以刪除）
    output$ai_analysis_results_backup <- renderUI({
