...
# 載入延遲載入資料庫模組
source("database/db_connection_lazy.R")

# 載入優化登入模組
source("modules/module_login_optimized.R")
...
source("modules/module_dna_multi_optimized.R")  # DNA 分析模組（優化版）

# 載入旗艦版四大板塊模組
source("modules/module_revenue_pulse.R")       # 營收脈能模組
source("modules/module_customer_acquisition.R") # 客戶增長模組
source("modules/module_customer_retention_new.R")   # 客戶留存模組（改進版）
source("modules/module_engagement_flow.R")      # 活躍轉化模組

# 載入時間序列分析模組
source("modules/module_time_series_analysis.R") # 時間序列分析模組
...
main_app_ui <- bs4DashPage(
  ...
  sidebar = bs4DashSidebar(
    ...
    div(class = "step-indicator",
        ...
        div(class = "step-item", id = "step_2", "2. DNA 分析")
    ),
    ...
    sidebarMenu(
      ...
      bs4SidebarHeader("VitalSigns 旗艦版"),
      bs4SidebarMenuItem(
        text = "營收脈能",
        tabName = "revenue_pulse",
        icon = icon("dollar-sign")
      ),
      bs4SidebarMenuItem(
        text = "客戶增長",
        tabName = "customer_acquisition",
        icon = icon("users")
      ),
      bs4SidebarMenuItem(
        text = "客戶留存",
        tabName = "customer_retention",
        icon = icon("user-check")
      ),
      bs4SidebarMenuItem(
        text = "活躍轉化",
        tabName = "engagement_flow",
        icon = icon("fire")
      ),
  ...
  body = bs4DashBody(
    ...
    bs4TabItems(
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            ...
            solidHeader = TRUE,
            elevation = 3,
            uploadModuleUI("upload1", enable_hints = TRUE)
          )
        )
      ),
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            title = "步驟 2：客戶 DNA 分析",
            status = "success",
            width = 12,
            solidHeader = TRUE,
            elevation = 3,
            dnaMultiModuleUI("dna_multi1", enable_hints = TRUE)
          )
        )
      ),
      ...
      bs4TabItem(
        tabName = "revenue_pulse",
        revenuePulseModuleUI("revenue_pulse1", enable_hints = TRUE)
      ),
      ...
      bs4TabItem(
        tabName = "engagement_flow",
        engagementFlowModuleUI("engagement_flow1", enable_hints = TRUE)
      ),
        ...
        fluidRow(
          bs4Card(
            ...
            div(
              ...
              img(src = "assets/icons/app_icon.png", width = "320px", alt = "Vital Signs Logo")
            ),
            ...
            p("我們是一套由 AI 驅動的精準行銷平台，協助品牌根據客戶特徵與行為數據，制定個人化行銷策略。我們整合 NLP、推薦系統與自動化分析以及統計和行銷理論，提供高效且可擴展的解決方案，協助行銷團隊更快達成轉換與黏著目標。", 
            ...
            div(
              ...
              tags$ul(
                ...
                tags$li(icon("bullseye"), " 客群分群建模（Segmentation Modeling）", style = "margin-bottom: 0.8rem; font-size: 1.1rem;"),
                tags$li(icon("brain"), " 意圖辨識與推薦系統（Intent Detection & Recommendation）", style = "margin-bottom: 0.8rem; font-size: 1.1rem;"),
                tags$li(icon("comments"), " 評論內容語意分析（Sentiment & Aspect Analysis）", style = "margin-bottom: 0.8rem; font-size: 1.1rem;"),
                tags$li(icon("chart-line"), " 行銷活動預測與績效追蹤（Campaign Forecasting & Tracking）", style = "margin-bottom: 0.8rem; font-size: 1.1rem;"),
                tags$li(icon("chart-bar"), " 多管道數據整合與儀表板（Omni-channel Dashboard & ETL）", style = "font-size: 1.1rem;")
              )
            ),
            
            h2("🤝 過去合作廠商", style = "color: #343a40; border-bottom: 2px solid #007bff; padding-bottom: 0.5rem;"),
            div(
              ...
              tags$a(
                ...
                target = "_blank",
                "美商駿旺 (Kitchen Mama)",
                style = "font-size: 1.2rem; color: #1976d2; text-decoration: none; font-weight: bold;"
              )
            ),
            
            hr(style = "margin: 2rem 0; border-color: #dee2e6;"),
            
            h2("📞 聯絡方式", style = "color: #343a40; border-bottom: 2px solid #007bff; padding-bottom: 0.5rem;"),
            div(
              style = "background: #fff3cd; padding: 1.5rem; border-radius: 8px; border-left: 4px solid #ffc107;",
...
server <- function(input, output, session) {
  ...
  # 上傳模組（整合hint系統）
  upload_mod <- uploadModuleServer("upload1", con_global, user_info, enable_hints = TRUE)
  observe({
  ...
  observeEvent(upload_mod$proceed_step(), {
    ...
    # DNA 分析模組（整合hint和prompt系統）
    dna_mod <- dnaMultiModuleServer("dna_multi1", con_global, user_info, upload_mod$dna_data, 
                                   chat_api = chat_api, enable_hints = TRUE, enable_prompts = TRUE)
    
    # 時間序列分析模組
    time_series_mod <- timeSeriesAnalysisServer(
      "time_series",
      raw_data = reactive({ upload_mod$dna_data() })
    )
    
    # VitalSigns 旗艦版四大板塊模組
    # 營收脈能模組（加入時間序列數據，整合hint系統）
    revenue_mod <- revenuePulseModuleServer("revenue_pulse1", con_global, user_info, dna_mod, 
                                           time_series_data = time_series_mod, enable_hints = TRUE,
                                           enable_gpt = TRUE, chat_api = chat_api)
    
    # 客戶增長模組（加入時間序列數據，整合hint系統）
    acquisition_mod <- customerAcquisitionModuleServer("customer_acquisition1", con_global, user_info, dna_mod, 
                                                       time_series_data = time_series_mod, enable_hints = TRUE,
                                                       enable_gpt = TRUE, chat_api = chat_api)
    
    # 客戶留存模組（整合hint系統和AI分析）
    retention_mod <- customerRetentionModuleServer("customer_retention1", con_global, user_info, dna_mod, 
                                                   enable_hints = TRUE, enable_gpt = TRUE, chat_api = chat_api)
    
    # 活躍轉化模組（整合hint系統和AI分析）
  ...
  output$welcome_user <- renderText({
    if (login_result$logged_in() && !is.null(user_info())) {
  ...
  output$db_status <- renderUI({
    if (login_result$logged_in() && !is.null(db_info())) {
      info <- db_info()
      div(
        style = sprintf("color: %s; padding: 8px; margin-right: 15px; background: rgba(255,255,255,0.1); border-radius: 5px;", info$color),
        span(info$icon, style = "margin-right: 5px;"),
        span(info$type, style = "font-weight: bold; margin-right: 5px;"),
        span(sprintf("(%s)", info$status), style = "font-size: 0.9em; opacity: 0.8;")
      )
    } else {
      div(
        style = "color: #ffc107; padding: 8px; margin-right: 15px; background: rgba(255,255,255,0.1); border-radius: 5px;",
        span("🔐", style = "margin-right: 5px;"),
        span("認證模式", style = "font-weight: bold;")
      )
    }
  })
