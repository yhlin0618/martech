...
# GPT Prompt 集中管理系統 - VitalSigns Premium
...
# VitalSigns 特定功能：銷售分析 prompt 快速函數
get_sales_analysis_prompt <- function(analysis_type, data, prompts_df = NULL) {
  # 根據分析類型準備變數
  variables <- switch(analysis_type,
    "trend" = list(
  ...
  # 對應的 var_id
  var_id <- switch(analysis_type,
    "trend" = "sales_trend_analysis",
    "performance" = "product_performance",
    "anomaly" = "anomaly_detection_report",
    analysis_type # 預設使用原始類型作為 var_id
  )
  
  return(prepare_gpt_messages(var_id, variables, prompts_df))
}