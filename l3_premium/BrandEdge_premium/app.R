      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            ...
            solidHeader = TRUE,
            elevation = 3,
            p("支援 Excel (.xlsx, .xls) 和 CSV 格式"),
            p("必要欄位：Variation（品牌）、Title（標題）、Body（評論內容）"),
            p("限制：最多1000則評論，20個品牌"),
            add_hint(
              fileInput("file", "選擇檔案", 
            ...
            actionButton("preview_btn", "預覽資料", class = "btn-primary"),
            br(), br(),
            DTOutput("preview_table"),
            br(),
            actionButton("confirm_upload", "確認上傳", class = "btn-success")
          )
        )
      ),
        ...
        fluidRow(
          bs4Card(
            ...
            div(class = "attr-selector",
              ...
              p("建議：一般產品10-15個，複雜產品20-30個")
            ),
            actionButton("generate_attributes", "產生屬性", 
                        class = "btn-primary btn-lg", 
                        icon = icon("magic")),
            br(), br(),
            uiOutput("attributes_display"),
            br(),
            conditionalPanel(
              condition = "output.attributes_display",
              actionButton("confirm_attributes", "確認屬性", 
                          class = "btn-success")
            )
          )
        )
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            ...
            solidHeader = TRUE,
            elevation = 3,
            p("系統將對每則評論的各個屬性進行1-5分評分"),
            numericInput("sample_size", "評分樣本數（每個品牌）：",
                        value = 50, min = 10, max = 100, step = 10),
            p("總評論數：", textOutput("total_reviews", inline = TRUE)),
            actionButton("start_scoring", "開始評分", 
                        class = "btn-warning btn-lg",
                        icon = icon("play")),
            br(), br(),
            uiOutput("scoring_progress"),
            br(),
            DTOutput("scoring_results"),
            br(),
            uiOutput("next_step_after_scoring")
          )
        )
      ),
...
server <- function(input, output, session) {
  ...
  # ──── 登入模組 ────────────────────────────────────────
  # 注意：不傳遞連接，讓模組內部使用 db_query 和 db_execute
  ...
  observeEvent(input$preview_btn, {
    ...
    tryCatch({
      ...
      if (!all(required_cols %in% names(df))) {
        showNotification("缺少必要欄位！", type = "error")
        return()
      }
